...
//     char            strbuf[BUF_MAX];
    ...
    bool            chains = 0;//align individual chains of complexes
    //
    ...
    std::string     nchns;//#chain alignments to output
    ...
    std::string     molt;//molecular type
    ...
    std::string     recalculate;//#top-scored superposition candidates to recalculate
    std::string     nbranches;//# final superposition branches to explore
    std::string     ndps;//#DP rounds for optimizing superposition search-resulted alignments
    bool            addsearchbyss = 0;//flag for adding search by ss
    bool            addchainlevelsearch = 0;//flag of including chain-level processing
    ...
    std::string     devmaxnqrs;//max #query chains allowed
    std::string     devmaxnqcpxs;//max #query complexes in the chunk
...
*/
    ...
    enum {
        ...
        gtaOpt_qrs, gtaOpt_rfs, gtaOpt_a2a, gtaOpt_sfx, gtaOpt_chains,
        gtaOpt_o, gtaOpt_c,
        //
        ...
        //
        gtaOpt_s, gtaOpt_2tm_score, gtaOpt_sort, gtaOpt_nhits, gtaOpt_nalns, gtaOpt_nchns,
        ...
        gtaOpt_window, gtaOpt_recalculate, gtaOpt_nbranches, gtaOpt_ndps,
        gtaOpt_add_search_by_ss, gtaOpt_add_chain_level_search, gtaOpt_no_detailed_search,
        gtaOpt_convergence, gtaOpt_speed,
        gtaOpt_cp, gtaOpt_mirror,
        //
        gtaOpt_cpu_threads_reading, gtaOpt_cpu_threads,
        gtaOpt_dev_queries_max_chains, gtaOpt_dev_queries_max_per_chunk,
    ...
    static struct myoption long_options[] = {
        ...
        {"chains", my_no_argument, gtaOpt_chains},
        //
        ...
        {"nchns", my_required_argument, gtaOpt_nchns},
        ...
        {"recalculate", my_required_argument, gtaOpt_recalculate},
        {"nbranches", my_required_argument, gtaOpt_nbranches},
        {"ndps", my_required_argument, gtaOpt_ndps},
        {"add-search-by-ss", my_no_argument, gtaOpt_add_search_by_ss},
        {"add-chain-level-search", my_no_argument, gtaOpt_add_chain_level_search},
        ...
        {"dev-queries-max-chains", my_required_argument, gtaOpt_dev_queries_max_chains},
        {"dev-queries-max-per-chunk", my_required_argument, gtaOpt_dev_queries_max_per_chunk},
        ...
        try {
            ...
            while(( c = mygetopt.GetNextOption( &myoptarg )) >= 0 ) {
                switch( c ) {
                    ...
                    case gtaOpt_chains: chains = 1; addchainlevelsearch = 1; break;
                    ...
                    case gtaOpt_nchns:  nchns = myoptarg; break;
                    ...
                    case gtaOpt_recalculate: recalculate = myoptarg; break;
                    case gtaOpt_nbranches:  nbranches = myoptarg; break;
                    case gtaOpt_ndps:       ndps = myoptarg; break;
                    case gtaOpt_add_search_by_ss: addsearchbyss = 1; break;
                    case gtaOpt_add_chain_level_search: addchainlevelsearch = 1; break;
                    ...
                    case gtaOpt_dev_queries_max_chains: devmaxnqrs = myoptarg; break;
                    case gtaOpt_dev_queries_max_per_chunk: devmaxnqcpxs = myoptarg; break;
    ...
    TRY
        ...
        if( !nchns.empty()) {
            if( mystring2int(c, nchns, "Invalid argument of option --nchns."))
                return EXIT_FAILURE;
            CLOPTASSIGN(O_NCHNS, c);
        }
        ...
        if(CLOptions::GetB_CHAINS() == 0) {
            if(CLOptions::GetI_TER() != CLOptions::istEOF &&
               CLOptions::GetI_TER() != CLOptions::istEND) {
                error("Invalid option --ter for complexes.");
                return EXIT_FAILURE;
            }
            if(CLOptions::GetI_SPLIT() != CLOptions::issaByMODEL &&
               CLOptions::GetI_SPLIT() != CLOptions::issaByChain) {
                error("Invalid option --split for complexes.");
                return EXIT_FAILURE;
            }
            if(CLOptions::GetI_SPLIT() == CLOptions::issaByMODEL &&
               CLOptions::GetI_TER() != CLOptions::istEOF) {
                error(std::string("Option --split=" + 
                    std::to_string(CLOptions::issaByMODEL) + 
                    " is only compatible with option --ter=0 for complexes.").c_str());
                return EXIT_FAILURE;
            }
        }
        ...
        if( !recalculate.empty()) {
            const char* errstr = "Invalid argument of option --recalculate.";
            if(mystring2int(c, recalculate, errstr)) return EXIT_FAILURE;
            if((c & (c - 1))) {error(errstr); return EXIT_FAILURE;}//power of 2
            CLOPTASSIGN(C_RECALCULATE, c);
        }
        ...
        if( !ndps.empty()) {
            if( mystring2int(c, ndps, "Invalid argument of option --ndps."))
                return EXIT_FAILURE;
            CLOPTASSIGN(C_NDPS, c);
            if(!c && CLOptions::GetC_NBRANCHES()) {
                warning("--ndps=0 also sets --nbranches=0.");
                CLOPTASSIGN(C_NBRANCHES, c);
            }
        }
        ...
        CLOPTASSIGN(C_ADDCHAINLEVELSEARCH, addchainlevelsearch);
        ...
        if( !speed.empty()) {
            ...
            int valgapcost = CLOptions::GetC_GAPCOST();
            int valtrigger = CLOptions::GetC_TRIGGER();
            int valseedrule = CLOptions::GetC_SEEDRULE();
            int valrecalculate = CLOptions::GetC_RECALCULATE();
            int valnbranches = CLOptions::GetC_NBRANCHES();
            int valndps = CLOptions::GetC_NDPS();
            int valconvergence = CLOptions::GetC_CONVERGENCE();
            int valnspatialits = CLOptions::GetH_N_SPATIAL_ITERATIONS();
            float valprescorefactor = CLOptions::GetH_PRESCORE_FACTOR();
            switch(c) {
                ...
                case 10: valdepth = CLOptions::csdMedium; valtrigger = 50;
                                    valrecalculate = 16; valnbranches = 1; valndps = 1; break;
                //
                case 11: valdepth = CLOptions::csdShallow; valtrigger = 20;
                                    valrecalculate = 16; valnbranches = 1; valndps = 1;
                                    valrefinement = coarseref; valconvergence = 2; break;
                case 12: valdepth = CLOptions::csdShallow; valtrigger = 50;
                                    valrecalculate = 16; valnbranches = 1; valndps = 1; valgapcost = 2;
                                    valrefinement = coarseref; valconvergence = 2; break;
                case 13: valdepth = CLOptions::csdShallow; valtrigger = 50;
                                    valrecalculate = 8; valnbranches = 0; valndps = 1; valgapcost = 2;
                                    valrefinement = coarseref; valconvergence = 2; break;
                case 14: valdepth = CLOptions::csdShallow; valtrigger = 50;
                                    valrecalculate = 4; valnbranches = 0; valndps = 1; valgapcost = 2;
                                    valrefinement = coarseref; valconvergence = 2; break;
                case 15: valdepth = CLOptions::csdShallow; valtrigger = 50;
                                    valrecalculate = 4; valnbranches = 0; valndps = 0; valgapcost = 2;
                                    valrefinement = coarseref; valconvergence = 2; break;
                case 16: valdepth = CLOptions::csdShallow; valtrigger = 50;
                                    valrecalculate = 2; valnbranches = 0; valndps = 0; valgapcost = 2;
                                    valrefinement = coarseref; valconvergence = 2;
                                    valnspatialits = 1; valprescorefactor = 0.5f; break;
            };
            CLOPTASSIGN(C_REFINEMENT, valrefinement);
            CLOPTASSIGN(C_DEPTH, valdepth);
            CLOPTASSIGN(C_GAPCOST, valgapcost);
            CLOPTASSIGN(C_TRIGGER, valtrigger);
            CLOPTASSIGN(C_SEEDRULE, valseedrule);
            CLOPTASSIGN(C_RECALCULATE, valrecalculate);
            CLOPTASSIGN(C_NBRANCHES, valnbranches);
            CLOPTASSIGN(C_NDPS, valndps);
            CLOPTASSIGN(C_CONVERGENCE, valconvergence);
            CLOPTASSIGN(C_ADDSEARCHBYSS, addsearchbyss);
            CLOPTASSIGN(H_N_SPATIAL_ITERATIONS, valnspatialits);
            CLOPTASSIGN(H_PRESCORE_FACTOR, valprescorefactor);
        }
        ...
        if(!devmaxnqrs.empty()) {
            if(mystring2int(c, devmaxnqrs, "Invalid argument of option --dev-queries-max-chains."))
                return EXIT_FAILURE;
            CLOPTASSIGN(DEV_QRS_MAX_CHAINS, c);
        }

        if(!devmaxnqcpxs.empty()) {
            if(mystring2int(c, devmaxnqcpxs, "Invalid argument of option --dev-queries-max-per-chunk."))
                return EXIT_FAILURE;
            CLOPTASSIGN(DEV_QRS_MAX_PER_CHUNK, c);
        }
        ...
        else if(CLOptions::GetB_CHAINS()) CLOPTASSIGN(DEV_MINRLEN, 20);
        ...
        if(!clsdblst.empty()) {
            ...
            // if(CLOptions::GetB_CHAINS() == 0) {
            //     error("Clustering of complexes not implemented.");
            //     return EXIT_FAILURE;
            // }
        }

        if(CLOptions::GetC_RECALCULATE() < CLOptions::GetC_NBRANCHES()) {
            error("--nbranches cannot be greater than --recalculate.");
            return EXIT_FAILURE;
        }
