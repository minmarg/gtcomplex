...
//
CLDEFINEASSIGNMENT( B_CHAINS, int, 0, value==0 || value==1, --chains);
...
CLDEFINEASSIGNMENT( O_NCHNS, int, 0, value>=0 && value<1000000, --nchns);
...
CLDEFINEASSIGNMENT( I_TER, int, istEND, value>=0 && value<istnIStructTerminator, --ter);
CLDEFINEASSIGNMENT( I_SPLIT, int, issaByChain, value>=0 && value<issanIStructSplitApproach, --split);
...
CLDEFINEASSIGNMENT( C_RECALCULATE, int, RECALCULATE_DEF_N, value>=RECALCULATE_MIN_N && value<=RECALCULATE_MAX_N, --recalculate);
CLDEFINEASSIGNMENT( C_NBRANCHES, int, csnNbranchesDefault, value>=0 && value<=16, --nbranches);
CLDEFINEASSIGNMENT( C_NDPS, int, csnNDPsDefault, value>=0 && value<=2, --ndps);
CLDEFINEASSIGNMENT( C_ADDSEARCHBYSS, int, 0, value==0 || value==1, --add-search-by-ss);
CLDEFINEASSIGNMENT( C_ADDCHAINLEVELSEARCH, int, 0, value==0 || value==1, --add-chain-level-search);
...
CLDEFINEASSIGNMENT( C_SPEED, int, 9, value>=0 && value<=16, --speed);
...
CLDEFINEASSIGNMENT( H_PRESCORE_FACTOR, float, 0.0f, value>=0.0f && value<=1.0f, --pre-score-factor);
//
...
CLDEFINEASSIGNMENT( DEV_QRS_MAX_CHAINS, int, DEF_QUERY_STRUCTURES_PER_CHUNK, 
    value>=MIN_QUERY_STRUCTURES_PER_CHUNK && value<=MAX_QUERY_STRUCTURES_PER_CHUNK, --dev-queries-max-chains)
CLDEFINEASSIGNMENT( DEV_QRS_MAX_PER_CHUNK, int, 1, value>=1 && value<=2, --dev-queries-max-per-chunk)
...
CLDEFINEASSIGNMENT( DEV_MINRLEN, int, 3, value>=3 && value<=32767, --dev-min-length)
