    ...
    float* __restrict__  scvCache)
{
    ...
    const int yoff1 = (qryndx * maxnsteps + complexstepnumber2/*sfragfctxndx*/) * dblen * nTDPAlignedPoss;
...
// 
template<int s2ScoreQ, int s2ScoreR>
__device__ __forceinline__
    ...
    float* __restrict__ alndatamem)
{
    ...
    if(threadIdx.x == 0) {
        ...
        atomicAdd(&alndatamem[mloc + s2ScoreQ], __fdividef(best, qrylenorg));
        atomicAdd(&alndatamem[mloc + s2ScoreR], __fdividef(gbest, dbstrlenorg));
    }
}
