...
// Instantiations
// 
#define INSTANTIATE_FindGaplessAlignedFragment(tpTFM_DINV, tpMAX_NCHAINS) \
    template __global__ void FindGaplessAlignedFragment<tpTFM_DINV,tpMAX_NCHAINS>( \
        const uint ndbCstrs, const uint ndbCposs, const uint maxnsteps, \
        const int arg1/* n1 */,const int arg2/* step */,const int arg3, \
        float* __restrict__ tmpdpdiagbuffers, \
        float* __restrict__ wrkmemaux);

INSTANTIATE_FindGaplessAlignedFragment(false,0);
INSTANTIATE_FindGaplessAlignedFragment(true,0);

INSTANTIATE_FindGaplessAlignedFragment(false,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
INSTANTIATE_FindGaplessAlignedFragment(true,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
