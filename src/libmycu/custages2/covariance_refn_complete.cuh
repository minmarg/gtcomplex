...
//
template<bool WRITEFRAGINFO, bool TFM_DINV, int MAX_NCHAINS = 0>
__global__ 
...
void SaveLocalBestScoreAndTM(
    ...
    const uint* sfragndx = NULL, const uint* sfragpos = NULL,
    uint* bsfragndx = NULL, uint* bsfragpos = NULL)
{
    const bool bwrite = (best < score);

    if(bwrite) {
        best = score;
        if(sfragndx && sfragpos && bsfragndx && bsfragpos) {
            *bsfragndx = *sfragndx; *bsfragpos = *sfragpos;
        }
    }
