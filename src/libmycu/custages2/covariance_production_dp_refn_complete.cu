...
// 
template<bool TFM_DINV, bool COMPLEX, int CPXTYPENUMBERSCT>
__global__ 
    ...
    float* __restrict__ alndatamem)
{
    ...
    enum {
        ttpndx = tawmvNAlnPoss + 1,//index to temporarily keep type value
...
//     __shared__ float tfmCache[twmvEndOfCCDataExt];//twmvEndOfCCDataExt>nTTranformMatrix
    ...
    int fraglen, cpxtype;//fragment position and length; complex type
    ...
    if(threadIdx.x == tawmvNAlnPoss) {
        ...
        uint mloct = ((qryndx * maxnsteps + CPXTYPENUMBERSCT) * nTAuxWorkingMemoryVars) * ndbCstrs;
        ccmCache[tawmvNAlnPoss] = wrkmemaux[mloc0 + tawmvNAlnPoss * ndbCstrs + dbstrndx];
        ccmCache[ttpndx] = wrkmemaux[mloct + tawmvNAlnPoss * ndbCstrs + dbstrndx];
    }
    ...
    for(uint fctxndx = sfragfctxndx; fctxndx < effnsteps; fctxndx += maxnsteps)
    {
        ...
        const uint sfragpos = sfragfct * sfragstep;
        ...
        if(fctxndx == 0) {//also implies sfragndx == 0
        ...
        for(int cit = 0; cit < nmaxconvit + 2; cit++)
        {
            ...
            //NOTE: no sync inside:
            SaveLocalBestScoreAndTM(
                best, ccmCache[1]/*score*/, tfmCache, tfmBest,
                &sfragndx, &sfragpos, &bsfragndx, &bsfragpos);
    ...
    const float g0 = GetD0fin(greaterlen, greaterlen, cpxtype);
    const float g02 = SQRD(g0);
    float gbest = best;//score calculated for the other structure

    if(0.0f <= best && qrylenorg != dbstrlenorg) {
        CalcScoresUnrl_DPRefined_Complete<CHCKDST_NOCHECK>(
            READCNST_CALC2,
            qryndx, ndbCposs, dbxpad, maxnsteps, sfragfctxndx, dbstrdst,
    ...
    SaveBestQRScoresAndTM_Complete<true/* WRITEFRAGINFO */,false/* CONDITIONAL */>(
        ...
        maxnsteps, sfragfctxndx, bsfragndx, bsfragpos,
...
// Instantiations
//
#define INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase1(tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT) \
    template __global__ void ProductionFragmentBasedDPAlignmentRefinementPhase1<tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT>( \
...
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase1(false,false,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase1(true,false,sfin_cpx_complextypenumber2);

INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase1(false,true,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase1(true,true,sfin_cpx_complextypenumber2);
// -------------------------------------------------------------------------
...
//     __shared__ float tfmCache[twmvEndOfCCDataExt];//twmvEndOfCCDataExt>nTTranformMatrix
    ...
    uint /*qrydst, */dbstrdst;
    enum {qrypos = 0, rfnpos = 0};
    int sfragpos, fraglen, cpxtype;//fragment position and length; complex type
...
// Instantiations
//
#define INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_fullsearch(tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT) \
    template __global__ void ProductionFragmentBasedDPAlignmentRefinementPhase2_fullsearch<tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT>( \
        const int nmaxconvit,const uint ndbCstrs,const uint ndbCposs,const uint dbxpad, \
...
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_fullsearch(false,false,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_fullsearch(true,false,sfin_cpx_complextypenumber2);

INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_fullsearch(false,true,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_fullsearch(true,true,sfin_cpx_complextypenumber2);
...
// Instantiations
//
#define INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2(tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT) \
    template __global__ void ProductionFragmentBasedDPAlignmentRefinementPhase2<tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT>( \
        const int nmaxconvit,const uint ndbCstrs,const uint ndbCposs,const uint dbxpad, \
        const uint nmaxsubfrags,const uint maxnfragfcts,const uint maxnsteps,const int sfragstep, \
        const int /*maxalnmax*/, \
        const float* __restrict__ tmpdpalnpossbuffer, \
...
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2(false,false,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2(true,false,sfin_cpx_complextypenumber2);

INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2(false,true,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2(true,true,sfin_cpx_complextypenumber2);
// =========================================================================
...
//     __shared__ float tfmCache[twmvEndOfCCDataExt];//twmvEndOfCCDataExt>nTTranformMatrix
    ...
    int qrylenorg, dbstrlenorg;//original query and reference lengths
    int qrylen, dbstrlen, cpxtype;//pseudo query and reference length, #matched positions; complex type
    //distances in positions to the beginnings of the query and reference structures:
    uint /*qrydst, */dbstrdst;
    enum {qrypos = 0, rfnpos = 0, tmpcslot = ttpndx + 1};
    ...
    if(COMPLEX) {
        //NOTE: dbstrndx and qryndx indicate complex sns for complexes!
        if(threadIdx.x == 0)
            GetDbComplexAddressLength(dbstrndx, ((int*)ccmCache)[tmpcslot+1], ((int*)ccmCache)[tmpcslot+0]);
#if (CUS1_TBINITSP_COMPLETEREFINE_XDIM >= 64)
        if(threadIdx.x == 32)
#else
        if(threadIdx.x == 0)
#endif
            GetQueryComplexAddressLength(qryndx, ((int*)ccmCache)[tmpcslot+3], ((int*)ccmCache)[tmpcslot+2]);
    } else if(threadIdx.x < 2) {
        //NOTE: pps2DLen and pps2DDist assumed to be adjacent: see PM2DVectorFields.h!
        //reuse ccmCache
...
// Instantiations
//
#define INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_logsearch(tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT) \
    template __global__ void ProductionFragmentBasedDPAlignmentRefinementPhase2_logsearch<tpTFM_DINV,tpCOMPLEX,tpCPXTYPENUMBERSCT>( \
        const int nmaxconvit,const uint ndbCstrs,const uint ndbCposs,const uint dbxpad, \
        const uint nmaxsubfrags,const uint maxnfragfcts,const uint maxnsteps,const int sfragstep, \
        const int /*maxalnmax*/, \
        const float* __restrict__ tmpdpalnpossbuffer, \
...
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_logsearch(false,false,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_logsearch(true,false,sfin_cpx_complextypenumber2);

INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_logsearch(false,true,sfin_cpx_complextypenumber2);
INSTANTIATE_ProductionFragmentBasedDPAlignmentRefinementPhase2_logsearch(true,true,sfin_cpx_complextypenumber2);
// =========================================================================
