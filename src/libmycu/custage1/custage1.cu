    ...
    float* __restrict__ alndatamem)
{
    ...
    dim3 nthrds_cv2init(CUS1_TBSP_SCORE_SET_XDIM,1,1);
    dim3 nblcks_cv2init(
    ...
    if(condition4filter1) {
        InitWrkmem2<<<nblcks_cv2init,nthrds_cv2init,0,streamproc>>>(
            ndbCstrs, maxnstepsmem2,  wrkmem2,  (float)CONVERGED_LOWTMSC_bitval/*value*/);
        MYCUDACHECKLAST;
    }
    ...
    InitScores<INITOPT_ALL><<<nblcks_scinit,nthrds_scinit,0,streamproc>>>(
        ndbCcpxs,  maxnsteps, minfraglen, false/*checkfragos*/,  wrkmemaux);
    MYCUDACHECKLAST;
    ...
    InitGTfmMatricesAndAData<<<nblcks_tfminit,nthrds_tfminit,0,streamproc>>>(
        nqycpxs, ndbCstrs, tfmmem, alndatamem);
    MYCUDACHECKLAST;
 }
...
// MAX_NCHAINS, max #complex chains for this stage to take effect;
// qystr1len, length of the largest query;
// dbstr1len, length of the largest reference;
// qystrnlen, length of the smallest query;
// dbstrnlen, length of the smallest reference;
    ...
    float* __restrict__ /*wrkmemtm*/)
{
    ...
    for(; n1 <= n2; n1 += stepinit * (int)maxnsteps)
    {
        ...
        if(symmetric)
            FindGaplessAlignedFragment<true/*TFM_DINV*/,MAX_NCHAINS>
            ...
            FindGaplessAlignedFragment<false/*TFM_DINV*/,MAX_NCHAINS>
                <<<nblcks_ficmpl,nthrds_ficmpl,0,streamproc>>>(
                    ndbCstrs, ndbCposs, maxnsteps, n1/*arg1*/, stepinit/*arg2*/, 0/*arg3*/,
                    tmpdpdiagbuffers, wrkmemaux);
    }
...
// Instantiations
// 
#define INSTANTIATE_stage1__stage1_findfrag2(tpMAX_NCHAINS) \
        template void stage1::stage1_findfrag2<tpMAX_NCHAINS>( \
                cudaStream_t streamproc, \
                int stepinit, \
                const uint maxnsteps, \
                const uint minfraglen, \
...
INSTANTIATE_stage1__stage1_findfrag2(CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
...
//             wrkmemaux, wrkmem2, tfmmem);
//     else
        stage1_refinefrag_helper2
            <CONDITIONAL,SECONDARYUPDATE,GRANDUPDATE,MAX_NCHAINS>(
...
// Instantiations
// 
#define INSTANTIATE_stage1__stage1_refinefrag( \
    tpCONDITIONAL,SECONDARYUPDATE,tpGRANDUPDATE,tpMAX_NCHAINS) \
        template void stage1::stage1_refinefrag \
            <tpCONDITIONAL,SECONDARYUPDATE,tpGRANDUPDATE,tpMAX_NCHAINS>( \
                std::map<CGKey,MyCuGraph>& stgraphs, \
                const int fragbydp, \
                const int nmaxconvit, \
...
INSTANTIATE_stage1__stage1_refinefrag(true,SECONDARYUPDATE_NOUPDATE,tawmvGrandBest,0);
INSTANTIATE_stage1__stage1_refinefrag(false,SECONDARYUPDATE_NOUPDATE,tawmvGrandBest,0);
INSTANTIATE_stage1__stage1_refinefrag(false,SECONDARYUPDATE_UNCONDITIONAL,tawmvGrandBest,0);
INSTANTIATE_stage1__stage1_refinefrag(false,SECONDARYUPDATE_CONDITIONAL,tawmvGrandBest,0);

INSTANTIATE_stage1__stage1_refinefrag(false,SECONDARYUPDATE_NOUPDATE,tawmvScore,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
...
///     }
    ...
    dim3 nblcks_arcmpl(ndbCstrs, myhdmin(nlocsteps, maxnsteps), nqystrs);

    if(fragbydp == stg1REFINE_INITIAL) {
        if(symmetric)
            FragmentBasedAlignmentRefinementC2C<true/*TFM_DINV*/,MAX_NCHAINS>
            ...
            FragmentBasedAlignmentRefinementC2C<false/*TFM_DINV*/,MAX_NCHAINS>
                <<<nblcks_arcmpl,nthrds_arcmpl,0,streamproc>>>(
                    nmaxconvit, ndbCstrs, ndbCposs,
                    nmaxsubfrags, maxnsteps, nlocsteps, sfragstep,
                    tmpdpdiagbuffers, wrkmemtmibest, wrkmemaux);
        ...
        if(symmetric)
            FragmentBasedDPAlignmentRefinementC2C<CONDITIONAL,true/*TFM_DINV*/,MAX_NCHAINS>
                <<<nblcks_arcmpl,nthrds_arcmpl,0,streamproc>>>(
                    false/*readlocalconv*/,
            ...
            FragmentBasedDPAlignmentRefinementC2C<CONDITIONAL,false/*TFM_DINV*/,MAX_NCHAINS>
                <<<nblcks_arcmpl,nthrds_arcmpl,0,streamproc>>>(
                    false/*readlocalconv*/,
                    nmaxconvit, ndbCstrs, ndbCposs, dbxpad,
                    nmaxsubfrags, maxnsteps, nlocsteps, sfragstep,
    ...
    SaveBestScoreAndTMAmongBests
        <WRITEFRAGINFO,GRANDUPDATE,false/*FORCEWRITEFRAGINFO*/,SECONDARYUPDATE>
        <<<nblcks_scmax,nthrds_scmax,0,streamproc>>>(
            ndbCstrs,  maxnsteps, myhdmin(nlocsteps, maxnsteps),
...
//     static const float gcosts[ncosts] = {-0.6f, 0.0f};
    ...
    for(int gi = 0; gi < ncosts; gi++)
    {
        ...
        {
            RunDP<COMPLETEAPPROACH,vANCHORRGN,vBANDED,GAP0,MAX_NCHAINS>(
            ...
            BtckToMatched32x<vANCHORRGN,vBANDED,false/*COMPLEX*/,MAX_NCHAINS>
            ...
            stage1_refinefrag
                <false/*CONDITIONAL*/,SECONDARYUPDATE_NOUPDATE,GRANDUPDATE,MAX_NCHAINS>(
                    stgraphs,
...
// Instantiations
// 
#define INSTANTIATE_stage1__stage1_dprefine( \
            tpGAP0,tpPRESCREEN,tpWRKMEMTM1,tpGRANDUPDATE,tpMAX_NCHAINS) \
        template void stage1::stage1_dprefine \
            <tpGAP0,tpPRESCREEN,tpWRKMEMTM1,tpGRANDUPDATE,tpMAX_NCHAINS>( \
                std::map<CGKey,MyCuGraph>& stgraphs, \
                cudaStream_t streamproc, \
                const int maxndpiters, \
...
INSTANTIATE_stage1__stage1_dprefine(true,true,false,tawmvGrandBest,0);
INSTANTIATE_stage1__stage1_dprefine(true,false,false,tawmvGrandBest,0);
INSTANTIATE_stage1__stage1_dprefine(false,false,false,tawmvGrandBest,0);
INSTANTIATE_stage1__stage1_dprefine(false,false,true,tawmvGrandBest,0);

INSTANTIATE_stage1__stage1_dprefine(true,false,false,tawmvScore,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
...
{
    if(COMPLETEAPPROACH)
        RunCompleteDP<ANCHORRGN,BANDED,GAP0,MAX_NCHAINS>(
            streamproc,
            gapcost, maxnsteps, nqystrs, ndbCstrs, 
            nqyposs, ndbCposs, qystr1len, dbstr1len, dbxpad,
            tmpdpbotbuffer, btckdata, wrkmemtmibest, wrkmemaux);
    else
        RunTDintensiveDP<ANCHORRGN,BANDED,GAP0,MAX_NCHAINS>(
            streamproc,
            gapcost, maxnsteps, nqystrs, ndbCstrs, 
            nqyposs, ndbCposs, qystr1len, dbstr1len, dbxpad,
    ...
    for(uint d = 0; d < nblkdiags; d++)
    {
        ExecDPwBtck3264x
            <ANCHORRGN,BANDED,GAP0,D02IND_SEARCH,
             false/*ALTSCTMS*/,false/*COMPLEX*/,MAX_NCHAINS>
