...
// === Assignment section ==================================================
// x-dimension for assigning chains between complexes
// NOTE: for proper calculation, always use size to match a power of two 
// when divided by the warp size, i.e., 32, 64, 128, 256, 512, and 1024)!
#define CUAP_MAKEASSIGNMENT_XDIM 128
...
// thread block x-dimension (>32) for calculating secondary TMscores;
#define CUDP_PRODUCTION_2TMSCORE_DIM_X 128
// x-dimension (>32) for calculating chain-specific scores (TMs & RMSDs);
#define CUDP_PRODUCTION_CS_SCORES_DIM_X 128
...
// x-dimension of thread block for checking score convergence in the chor stage:
#define CUS1_CHOR_CONV_SCORE_XDIM 1024
...
// thread block dimensions for finding the maximum among DP scores 
// calculated for one fragment factor (x-dim corresponds to #structures):
#define CUS1_TBSP_DPSCORE1_MAX_XDIM 128
...
// x,y thread block dimensions for calculating initial cross-covariance matrices
// based on local structural alignment:
#define CUSF_TBSP_INITIAL_ALN_CCM_XDIM 32
#define CUSF_TBSP_INITIAL_ALN_CCM_YDIM 6
...
// cache size for #max complex chains
#define CUSF2_TBSP_INDEX_SCORE_COMPLEX_NCHAINS_CACHE_SIZE 128
...
// dimension x for reformatting matched positions (coordinates);
#define CUDP_REFORMAT_MATCHED_DIM_X 32 //64
// dimension y thread block for reformatting matched positions;
#define CUDP_REFORMAT_MATCHED_DIM_Y 6
