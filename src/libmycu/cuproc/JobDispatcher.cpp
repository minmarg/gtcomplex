    ...
    for(size_t n = 0; n < nthreads; n++) {
        std::unique_ptr<TdDataReader> tdr(
            new TdDataReader(
                ...
                references->GetMasterFileCounter(),
                n/*ndxstartwith*/,
                nthreads/*ndxstep*/,
    ...
    int nagents)
{
    ...
    const int chunkncpxs = CLOptions::GetDEV_QRS_MAX_PER_CHUNK();
...
bool JobDispatcher::GetReferenceData(
    ...
    size_t ntotqcpxs, int queryblocks,
    ...
    char* const * const * const querypmends)
{
    ...
    std::function<void()> lfGetDataBcst = 
        [this,ntotqcpxs,queryblocks,querypmbegs,querypmends]() {
        std::for_each(readers_.begin(), readers_.end(),
            [ntotqcpxs,queryblocks,querypmbegs,querypmends]
                (std::unique_ptr<TdDataReader>& p)
            {
                if(p)
                    p->Notify(
...
void JobDispatcher::Run()
{
    ...
    std::vector<int> qrycpxsernrbegs;//serial numbers for query complexes
    std::vector<int> qrybuffsrtndxs;//sorted indices of query data packets
    ...
    size_t qrschunknstrs = (size_t)
        (complexes? CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK(): CLOptions::GetDEV_QRS_MAX_PER_CHUNK());
    ...
    try {
        ...
        const size_t nbuffers_qryreader = //hostworkers_.size()-aligned
            (mymax((size_t)JDSP_MAX_N_BUFFERS_QRY_READER, hostworkers_.size()) / hostworkers_.size()) *
            hostworkers_.size();

        if((nbuffers_qryreader % hostworkers_.size()) != 0)
            throw MYRUNTIME_ERROR(preamb + "# query data packets not aligned properly.");

        CreateQrsReader( 
            -1,//query length unlimited (limited by max total length)
            CLOptions::GetIO_FILEMAP(),
            nbuffers_qryreader,//2 * hostworkers_.size(),
        ...
        querydescs.reset(new char**[nbuffers_qryreader]);
        querypmbegs.reset(new char**[nbuffers_qryreader]);
        querypmends.reset(new char**[nbuffers_qryreader]);
        queryndxpmbegs.reset(new char**[nbuffers_qryreader]);
        queryndxpmends.reset(new char**[nbuffers_qryreader]);
        qrysernrbegs.resize(hostworkers_.size(), 0);
        qrycpxsernrbegs.resize(hostworkers_.size(), 0);
        qrybuffsrtndxs.resize(nbuffers_qryreader);
        qrstscnt.reset(new TSCounterVar*[nbuffers_qryreader]);
        ...
        size_t ntotqstrs = 0, ntotqcpxs = 0;
        ...
        for(int qblk = 0;;)
        {
            ...
            memset(querydescs.get(), 0, nbuffers_qryreader * sizeof(void*));
            memset(querypmbegs.get(), 0, nbuffers_qryreader * sizeof(void*));
            memset(querypmends.get(), 0, nbuffers_qryreader * sizeof(void*));
            memset(queryndxpmbegs.get(), 0, nbuffers_qryreader * sizeof(void*));
            memset(queryndxpmends.get(), 0, nbuffers_qryreader * sizeof(void*));
            memset(qrstscnt.get(), 0, nbuffers_qryreader * sizeof(void*));

            size_t nqrybffs = 0;
            for(; nqrybffs < nbuffers_qryreader; nqrybffs++)
            {
                if(!GetDataFromReader(
                    GetQrsReader(),
                    querydescs[nqrybffs], querypmbegs[nqrybffs], querypmends[nqrybffs],
                    queryndxpmbegs[nqrybffs], queryndxpmends[nqrybffs],
                    qrstscnt[nqrybffs], &qrslastchunk))
                    //leave if no data
                    break;
            }
            ...
            //indices from 0 to #buffers
            std::iota(qrybuffsrtndxs.begin(), qrybuffsrtndxs.end(), 0);
            std::sort(qrybuffsrtndxs.begin(), qrybuffsrtndxs.begin() + nqrybffs,
                [&querypmbegs,&querypmends](size_t n1, size_t n2) {
                    return //sort by length in descending order
                        PMBatchStrData::GetNoPosits(querypmbegs[n1], querypmends[n1]) > 
                        PMBatchStrData::GetNoPosits(querypmbegs[n2], querypmends[n2]);
                });


            for(size_t qbi = 0; qbi < nqrybffs; qbi += hostworkers_.size())
            {
                newsetqrs = true;
                valnwparams.clear();

                for(size_t wrk = 0; wrk < hostworkers_.size() && qbi + wrk < nqrybffs; wrk++)
                {
                    int ndx = qrybuffsrtndxs[qbi + wrk];
                    if((int)nqrybffs <= ndx)
                        throw MYRUNTIME_ERROR(preamb + "Invalid query data packet index.");
                    qrysernrbegs[wrk] = ntotqstrs;
                    qrycpxsernrbegs[wrk] = ntotqcpxs;

                    size_t nqcpxs = PMBatchStrData::GetNoComplexes(querypmbegs[ndx], querypmends[ndx]);
                    size_t nqstrs = PMBatchStrData::GetNoStructs(querypmbegs[ndx], querypmends[ndx]);
                    size_t nqposs = PMBatchStrData::GetNoPosits(querypmbegs[ndx], querypmends[ndx]);

                    MYMSGBEGl(1)
                        sprintf(msgbuf,"%s[*********] Query BLOCK No. %d: "
                                "%zu positions (%zu chns) assigned to worker %zu",
                    ...
                    tscnt = tscntNext)
                {
                    ...
                    size_t nCstrs = datapresent? PMBatchStrData::GetNoStructs(bdbCpmbeg, bdbCpmend): 0;
                    size_t nposits = datapresent? PMBatchStrData::GetNoPosits(bdbCpmbeg, bdbCpmend): 0;
                    ...
                    if(!datapresent && chkno == 0)
                        for(size_t w = 0; w < hostworkers_.size(); w++)
                            //NOTE: hostworkers_.size() <= nbuffers_qryreader!
                            qrstscnt[ qrybuffsrtndxs[qbi + w] ]->reset();
                    ...
                    {
                        //NOTE: hostworkers_.size() <= nbuffers_qryreader!
                        int ndx = qrybuffsrtndxs[(int)qbi + tid];
                        ProcessPart(
                            tid, chkno+1, lastchunk,
                            newsetqrs,
                            qrycpxsernrbegs[tid], qrysernrbegs[tid],
                            scorethld,
                            queryndxpmbegs[ndx], queryndxpmends[ndx],
                            (const char**)querydescs[ndx], querypmbegs[ndx], querypmends[ndx],
...
void JobDispatcher::RunClust()
{
    ...
    size_t qrschunkdatalen = totqrsposs;
    size_t qrschunkdatasize = PMBatchStrData::GetPMDataSizeUB(qrschunkdatalen);
    // size_t qrschunknstrs = (size_t)(CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK());
    size_t qrschunknstrs = (size_t)
        (complexes? CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK(): CLOptions::GetDEV_QRS_MAX_PER_CHUNK());
    size_t qrschunkncpxs = (size_t)CLOptions::GetDEV_QRS_MAX_PER_CHUNK();
    ...
    try {
        ...
        //create the results writing thread
        CreateClustWriter(output_, GetClustList(), devnames, qrschunkncpxs, nagents);
        ...
        for(int qblk = 0;;)
        {
            ...
            for(; wrk < (int)hostworkers_.size(); wrk++) {
                qrysernrbegs[wrk] = (int)ntotqstrs;
                qrycpxsernrbegs[wrk] = (int)ntotqcpxs;
                ...
                size_t nqcpxs = PMBatchStrData::GetNoComplexes(querypmbegs[wrk], querypmends[wrk]);
                ...
                MYMSGBEGl(1)
                    sprintf(msgbuf,"%s[*********] Query BLOCK No. %d: "
                            "%zu chns (%zu pos.) (worker %d) / %zu processed",
                ...
                if(nqstrs)
                    valnwparams.push_back(
                        // std::make_tuple(nqstrs, qrysernrbegs[wrk], qrysernrbegs[wrk]+nqstrs-1));
                        std::make_tuple(nqcpxs, qrycpxsernrbegs[wrk], qrycpxsernrbegs[wrk] + nqcpxs - 1));
            }
