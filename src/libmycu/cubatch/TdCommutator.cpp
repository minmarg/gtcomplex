...
void TdCommutator::ProcessBlock(CuBatch& cbpc)
{
    ...
    char msgbuf[BUF_MAX];
    int nqcpxs = PMBatchStrData::GetNoComplexes(querypmbeg_, querypmend_);
    int nqueries = PMBatchStrData::GetNoStructs(querypmbeg_, querypmend_);
    int qrysernrend = qrysernrbeg_ + nqueries - 1;
    int qrycpxsernrend = qrycpxsernrbeg_ + nqcpxs - 1;
    ...
    if(!lastchunk_) {
        ...
        if(clustwriter_) clustwriter_->IncreaseQueryNParts(qrycpxsernrbeg_, qrycpxsernrend);
    }

    if(clustwriter_ && !bdbCinf)
        clustwriter_->PushPartOfResults(
            qrycpxsernrbeg_, qrysernrbeg_, 0/*nqyposs (unused)*/, nqueries,
