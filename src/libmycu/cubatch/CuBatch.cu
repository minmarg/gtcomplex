...
#include "libmycu/custage1/custage2_complex.cuh"
#include "libmycu/custage1/custage_ssrr.cuh"
#include "libmycu/custage1/custage_ssrr_complex.cuh"
#include "libmycu/custage1/custage_chor_complex.cuh"
#include "libmycu/custage1/custage_frg3.cuh"
#include "libmycu/custage1/custage_fin_complex.cuh"
    ...
    limit_beg_results_(0UL)
{
    ...
    const int maxnquerychains = CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK();
    const size_t szstats = maxnquerychains * nDevGlobVariables;
    ...
    unsigned int* globvarsbuf)
{
    ...
    static const int complexes = 1;//consistent; (CLOptions::GetB_CHAINS() == 0);
    ...
    if(complexes) {
        //similarity prescreening across complexes: all chains of a reference
        //complex with at least one instance above the threshold are set for
        //further processing;
        //NOTE: complex fields are updated simultaneously in the same kernel:
        if(passno == 0)
            //passno == 0: implies convergence flags have been set for individual chains;
            MakeCandidateComplexList<<<nblcks_filter,nthrds_filter,0,streamproc>>>(
                nqycpxs, ndbCmpxs, nqystrs, ndbCstrs, maxnsteps, maxnstepsmem2,
                auxwrkmemory, wrkmemory2, globvarsbuf);
        else
            //passno > 0: implies convergence flags have been set for complexes!
            MakeCandidateComplexList2<<<nblcks_filter,nthrds_filter,0,streamproc>>>(
                nqycpxs, ndbCmpxs, nqystrs, ndbCstrs, maxnsteps,  auxwrkmemory, globvarsbuf);
        MYCUDACHECKLAST;
    }
    ...
    MakeDbCandidateList<<<nblcks_filter,nthrds_filter,0,streamproc>>>(
        (complexes? nqycpxs: nqystrs), ndbCstrs, maxnsteps,  auxwrkmemory, globvarsbuf);
    MYCUDACHECKLAST;
    ...
    ndbCmpxs2 = PMBatchStrData::GetNoComplexes(bdbCpmbeg, bdbCpmend);
    ...
    if(ndbCstrs2) {
        ...
        PMBatchStrData::GetMinMaxLengths(bdbCpmbeg, bdbCpmend, dbstrnlen2, dbstr1len2);
    }
    ...
    MYMSGBEGl(msglev)
        sprintf(msgbuf,"%6c%zu chains proceed (%zu pos.)",' ',ndbCstrs2,ndbCposs2);
        MYMSG(msgbuf,1);
    MYMSGENDl
    MYMSGBEGl(msglev+1)
...
// =========================================================================
// ProcessBlockHelper: structure alignment algorithm for numbers of 
// query and reference structures on device;
void CuBatch::ProcessBlock(
    ...
    TSCounterVar* cnt)
{
    ProcessBlockComplexHelper(
        scorethld, qrycpxsernrbeg, qrysernrbeg,
...
// -------------------------------------------------------------------------
// ProcessBlockComplexHelper: implement structure alignment algorithm for 
// part of query and reference structures on device (complex version);
// scorethld, TM-score threshold;
// querydesc, descriptions of query structures;
// qrycpxsernrbeg, serial number of the first query complex in the chunk;
...
// bdbCpmend, terminal addresses of structures read from the database;
//
void CuBatch::ProcessBlockComplexHelper(
    float scorethld,
    int qrycpxsernrbeg,
    int qrysernrbeg,
    const char** querydesc,
    ...
    TSCounterVar* cnt)
{
    MYMSG("CuBatch::ProcessBlockComplexHelper", 4);
    static const std::string preamb = "CuBatch::ProcessBlockComplexHelper: ";
    static const float clstcoverage = CLOptions::GetB_CLS_COVERAGE();
    static const float seqsimthrscore = CLOptions::GetP_PRE_SIMILARITY();
    static const float prescore = CLOptions::GetP_PRE_SCORE();
    const int bchains = CLOptions::GetB_CHAINS();
    const int depth = CLOptions::GetC_DEPTH();
    const int nbranches = CLOptions::GetC_NBRANCHES();
    const int ndps = CLOptions::GetC_NDPS();
    ...
    const int stepinit = 1;//(depth == CLOptions::csdShallow)? 5: 1;
...
// #endif
    ...
    const size_t nqycmpxs = PMBatchStrData::GetNoComplexes(querypmbeg, querypmend);
    ...
    size_t ndbCmpxs = PMBatchStrData::GetNoComplexes(bdbCpmbeg, bdbCpmend);
    ...
    size_t qystr1len, qycpx1len;//length of the largest query structure/complex
    size_t dbstr1len, dbcpx1len;//length of the largest reference structure/complex
    size_t qystrnlen, qycpxnlen;//smallest length among queries
    size_t dbstrnlen, dbcpxnlen;//smallest length among references

    PMBatchStrData::GetMinMaxLengths(querypmbeg, querypmend, qystrnlen, qystr1len);
    PMBatchStrData::GetMinMaxLengths(bdbCpmbeg, bdbCpmend, dbstrnlen, dbstr1len);

    PMBatchStrData::GetMinMaxComplexLengths(querypmbeg, querypmend, qycpxnlen, qycpx1len);
    PMBatchStrData::GetMinMaxComplexLengths(bdbCpmbeg, bdbCpmend, dbcpxnlen, dbcpx1len);

    size_t ndbxposs = ndbCposs;

    if((size_t)maxnquerychains < nqystrs)
    ...
    try {
        ...
        //filtering condition for clustering:
        bool condition4filter0 = clustwriter_;
        ...
        stage1::preinitialize1(
            ...
            maxnsteps, minfraglen, nqycmpxs, ndbCmpxs, nqystrs, ndbCstrs, nqyposs, ndbCposs,
            wrkmemorytmibest, auxwrkmemory, wrkmemory2, tfmmemory, alndatamemory
        );
        ...
        if(condition4filter0) {
            ...
            CheckMaxCoverage<<<nblcks_covchk,nthrds_covchk,0,streamproc>>>(
                clstcoverage, qrycpxsernrbeg + (int)nqycmpxs, ndbCstrs, maxnstepsmem2, wrkmemory2
            );
        }
        //--- EBD FILTER0 --------------------------------------------------
        ...
        size_t ndbCmpxs1 = ndbCmpxs;
        size_t dbstr1len1 = dbstr1len;
        size_t dbstrnlen1 = dbstrnlen;
        size_t dbcpx1len1 = dbcpx1len;
        size_t dbcpxnlen1 = dbcpxnlen;
        ...
        if(condition4filter1) {
            ...
            VerifyAlignmentScore<<<nblcks_seqaln,nthrds_seqaln,0,streamproc>>>(
                seqsimthrscore, nqystrs, ndbCstrs, maxnstepsmem2, wrkmemory2
            );
        }
        ...
        if(condition4filter0 || condition4filter1) {
            FilteroutReferences(
                ...
                nqyposs, nqystrs, nqycmpxs, ndbCposs, ndbCstrs, ndbCmpxs, dbstr1len, dbstrnlen, dbxpad,
                maxnsteps, ndbCposs1, ndbCstrs1, ndbCmpxs1, dbstr1len1, dbstrnlen1, dbxpad1,
                tmpdpdiagbuffers, tfmmemory, auxwrkmemory, wrkmemory2, globvarsbuf);

            PMBatchStrData::GetMinMaxComplexLengths(bdbCpmbeg, bdbCpmend, dbcpxnlen1, dbcpx1len1);
        }
        //--- END FILTER ---------------------------------------------------
        ...
        const uint maxnqrychains = (uint)PMBatchStrData::GetMaxNComplexChains(querypmbeg, querypmend);
        const uint maxnrfnchains = (uint)PMBatchStrData::GetMaxNComplexChains(bdbCpmbeg, bdbCpmend);

        const uint minnqrychains = (uint)PMBatchStrData::GetMinNComplexChains(querypmbeg, querypmend);
        const uint minnrfnchains = (uint)PMBatchStrData::GetMinNComplexChains(bdbCpmbeg, bdbCpmend);

        if(ndbCstrs1 && ndbCposs1 && addchainlevelsearch) {
            //run the stage for finding suboptimal superpositions from individual chain processing
            stage_chor_complex::run_stage_chor_complex(
                stgraphs, streamproc, scorethld, prescore, stepinit, 2/*maxndpiters*/,
                maxnsteps, minfraglen, nqycmpxs, ndbCmpxs1, nqystrs, ndbCstrs1, nqyposs, ndbCposs1,
                qycpx1len, dbcpx1len1, qystr1len, dbstr1len1, qystrnlen, dbstrnlen1, dbxpad1,
                maxnqrychains, maxnrfnchains, minnqrychains, minnrfnchains, //
        ...
        const int stage3asfilter = (0.0f < prescore && depth < (int)CLOptions::csdMedium && !bchains);
        const int filtdepth = stage3asfilter? (int)CLOptions::csdMedium: depth;
        const int filtnbranches = nbranches;//nbranches irrelevant when stage3asfilter is true
        const int filtsimthreshold = simthreshold;//stage3asfilter? (int)CLOptions::cstTriggerDefault: simthreshold;

        //run the first stage for finding transformation matrices
        if(ndbCstrs1 && ndbCposs1)
            stagefrg3::run_stagefrg3_complexes(
                stgraphs, streamproc,
                stage3asfilter, filtdepth, filtnbranches, filtsimthreshold, ndps/*maxndpiters*/,
                maxnsteps, minfraglen, scorethld, prescore,
                nqycmpxs, ndbCmpxs1, nqystrs, ndbCstrs1, nqyposs, ndbCposs1,
                qycpx1len, dbcpx1len1, qystr1len, dbstr1len1, qystrnlen, dbstrnlen1, dbxpad1,
        ...
        size_t ndbCmpxs2 = ndbCmpxs1;
        size_t dbstr1len2 = dbstr1len1;
        size_t dbstrnlen2 = dbstrnlen1;
        size_t dbcpx1len2 = dbcpx1len1;
        size_t dbcpxnlen2 = dbcpxnlen1;
        size_t dbxpad2 = dbxpad1;

        uint maxnrfnchains2 = maxnrfnchains;

        if(0.0f < prescore && ndbCstrs1 && ndbCposs1) {
            FilteroutReferences(
                1/*passno*/,
                streamproc, bdbCdesc, bdbCpmbeg, bdbCpmend,
                nqyposs, nqystrs, nqycmpxs, ndbCposs1, ndbCstrs1, ndbCmpxs1, dbstr1len1, dbstrnlen1, dbxpad1,
                maxnsteps, ndbCposs2, ndbCstrs2, ndbCmpxs2, dbstr1len2, dbstrnlen2, dbxpad2,
                tmpdpdiagbuffers, tfmmemory, auxwrkmemory, wrkmemory2, globvarsbuf);

            PMBatchStrData::GetMinMaxComplexLengths(bdbCpmbeg, bdbCpmend, dbcpxnlen2, dbcpx1len2);
            maxnrfnchains2 = (uint)PMBatchStrData::GetMaxNComplexChains(bdbCpmbeg, bdbCpmend);
        }
        //--- END FILTER ---------------------------------------------------
        ...
        //run the first stage for finding transformation matrices
        if(ndbCmpxs2 && ndbCstrs2 && ndbCposs2 && stage3asfilter)
            stagefrg3::run_stagefrg3_complexes(
                stgraphs, streamproc,
                0/*stage3asfilter; no filter here*/, depth, nbranches, simthreshold, 2/*maxndpiters*/,
                maxnsteps, minfraglen, scorethld, 0.0f/*prescore; filtered already*/,
                nqycmpxs, ndbCmpxs2, nqystrs, ndbCstrs2, nqyposs, ndbCposs2,
                qycpx1len, dbcpx1len2, qystr1len, dbstr1len2, qystrnlen, dbstrnlen2, dbxpad2,
                maxnqrychains, maxnrfnchains2,//
                scores, tmpdpdiagbuffers, tmpdpbotbuffer, tmpdpalnpossbuffer, maxcoordsbuf, btckdata,
                wrkmemory, wrkmemoryccd, wrkmemorytmalt, wrkmemorytm, wrkmemorytmibest,
                auxwrkmemory, wrkmemory2, tfmmemory,
                globvarsbuf
        ...
        if(ndbCmpxs2 && ndbCstrs2 && ndbCposs2) {
        ...
        if(ndbCmpxs2 && ndbCstrs2 && ndbCposs2 && addsearchbyss) {
            //run superposition search by a combination of secondary structure and
            //sequence similarity
            stage_ssrr_complex::run_stage_ssrr_complex<true/*USESEQSCORING*/>(
                stgraphs, streamproc, scorethld, prescore, 2/*maxndpiters*/,
                maxnsteps, minfraglen, nqycmpxs, ndbCmpxs2, nqystrs, ndbCstrs2, nqyposs, ndbCposs2,
                qycpx1len, dbcpx1len2, qystr1len, dbstr1len2, qystrnlen, dbstrnlen2, dbxpad2,
                maxnqrychains, maxnrfnchains2,//
                scores, tmpdpdiagbuffers, tmpdpbotbuffer, tmpdpalnpossbuffer, maxcoordsbuf, btckdata,
        ...
        if(ndbCmpxs2 && ndbCstrs2 && ndbCposs2) {
            stage2_complex::run_stage2_complex<false/* GAP0 */, true/* USESS */, D02IND_DPSCAN>(
        ...
        if(ndbCmpxs2 && ndbCstrs2 && ndbCposs2) {
            stage2_complex::run_stage2_complex<true/* GAP0 */, false/* USESS */, D02IND_SEARCH>(
        ...
        if(ndbCmpxs2 && ndbCstrs2 && ndbCposs2) {
            stagefin_complex::run_stagefin_complex(
        ...
        if(clustwriter_)
            TransferResultsForClustering(
                qrycpxsernrbeg, qrysernrbeg,  nqyposs, nqystrs,
                querydesc, querypmbeg, querypmend,
                bdbCdesc, bdbCpmbeg, bdbCpmend,
...
void CuBatch::TransferResultsForClustering(
    int qrycpxsernrbeg, int qrysernrbeg,
    size_t nqyposs, size_t nqystrs,
    const char** querydesc,
    char** querypmbeg,
    char** querypmend,
