...
// -------------------------------------------------------------------------
// regvalue, register value to check;
// smembuf, smem buffer for temporary values, one pwe warp;
// return #registers with the value VALUE;
// 
template<int VALUE>
__device__ __forceinline__
int GetNoValuesAcrossWarps(int regvalue, float* smembuf)
{
    //sums in each warp
    regvalue = (regvalue == VALUE);
    regvalue = mywarpreducesum(regvalue);
    if((threadIdx.x & (lWarpsize - 1)) == 0) smembuf[threadIdx.x >> llog2warpsize] = regvalue;
    __syncthreads();
    regvalue = 0;
    if(threadIdx.x < (blockDim.x >> llog2warpsize)) regvalue = smembuf[threadIdx.x];
    regvalue = mywarpreducesum(regvalue);
    return regvalue;
}
    ...
    float* __restrict__ wrkmemaux)
{
    ...
    for(int i = 0; i < CUSF_TBSP_INDEX_SCORE_XFCT; i++) {
        ...
        {
            float rx, ry, rz;
            bestrnx = INVALID;//reference index of the position nearest to a query atom
        ...
        if(SECSTRFILT == 1 && blockIdx.x == 0 && trigger) {
            __syncthreads();
            //reuse stack trtStack; enough space by definition!
            ninvalids += GetNoValuesAcrossWarps<INVALID>(bestrnx, trtStack);
            if(i + 1 < CUSF_TBSP_INDEX_SCORE_XFCT) __syncthreads();
        }
    }

    if(SECSTRFILT == 1 && blockIdx.x == 0 && threadIdx.x == 0 &&
       (1.f - trigger) * (float)myhdmin((int)blockDim.x * CUSF_TBSP_INDEX_SCORE_XFCT, (qrylen - qrypos)) <
       (float)ninvalids)
    {
        uint mloc = ((qryndx * maxnsteps + sfragfct) * nTAuxWorkingMemoryVars + tawmvConverged) * ndbCstrs;
        wrkmemaux[mloc + dbstrndx] = CONVERGED_SCOREDP_bitval;
    }
}
    ...
    float* __restrict__ wrkmemaux)
{
    ...
    for(int i = 0; i < CUSF_TBSP_INDEX_SCORE_XFCT; i++) {
        ...
        {
            float qx, qy, qz;
            bestqnx = INVALID;//query index of the position nearest to a reference atom
        ...
        if(SECSTRFILT == 1 && blockIdx.x == 0 && trigger) {
            __syncthreads();
            //reuse stack trtStack; enough space by definition!
            ninvalids += GetNoValuesAcrossWarps<INVALID>(bestqnx, trtStack);
            if(i + 1 < CUSF_TBSP_INDEX_SCORE_XFCT) __syncthreads();
        }
    }

    if(SECSTRFILT == 1 && blockIdx.x == 0 && threadIdx.x == 0 &&
       (1.f - trigger) * (float)myhdmin((int)blockDim.x * CUSF_TBSP_INDEX_SCORE_XFCT, (dbstrlen - rfnpos)) <
       (float)ninvalids)
    {
        uint mloc = ((qryndx * maxnsteps + sfragfct) * nTAuxWorkingMemoryVars + tawmvConverged) * ndbCstrs;
        wrkmemaux[mloc + dbstrndx] = CONVERGED_SCOREDP_bitval;
    ...
    float* __restrict__ wrkmemaux)
{
    ...
    ProduceAlignmentUsingIndex2QueryHelper<SECSTRFILT>(
        ...
        WRTNDX, windowsize, 
        0/*seedapproachstruct*/, (depth), trigger, nqystrs, ndbCstrs, ndbCposs,
...
#define INSTANTIATE_ProduceAlignmentUsingIndex2(tpSECSTRFILT) \
    template __global__ void ProduceAlignmentUsingIndex2<tpSECSTRFILT>( \
        ...
        const int windowsize, const int depth, const float trigger, const uint nqystrs, \
...
// 
template<int SECSTRFILT, int COMPLEX>
__global__
    ...
    float* __restrict__ wrkmemaux)
{
    ...
    if(COMPLEX) {
        if(threadIdx.x == 0) {
            GetDbComplexAddressLength(dbstrndx, ((int*)dSMEM)[1], ((int*)dSMEM)[0]);
            if(SECSTRFILT) ((int*)dSMEM)[8] = GetDbStrField<INTYPE,pcx2DType>(dbstrndx);
        }

        if((CUSF_TBSP_INDEX_SCORE_XDIM <= lWarpsize && threadIdx.x == 1) ||
           (CUSF_TBSP_INDEX_SCORE_XDIM  > lWarpsize && threadIdx.x == lWarpsize)) {
            GetQueryComplexAddressLength(qryndx, ((int*)dSMEM)[3], ((int*)dSMEM)[2]);
            if(SECSTRFILT) ((int*)dSMEM)[9] = GetQueryStrField<INTYPE,pcx2DType>(qryndx);
        }
    }
    else if(threadIdx.x < 2) {
        GetDbStrLenDst(dbstrndx, (int*)dSMEM);
        GetQueryLenDst(qryndx, (int*)dSMEM + 2);
    }
    ...
    if(qrylen < dbstrlen) {
        if(notusingss)
            ProduceAlignmentUsingIndex2ReferenceHelper<0>(
                ...
                qrylen, dbstrlen, qrydst, dbstrdst,
                WRTNDX, windowsize,
                seedapproachstruct, (depth), trigger, nqystrs, ndbCstrs, ndbCposs,
                maxnsteps, qryfragfct, rfnfragfct,
                wrkmemtm, tmpdpalnpossbuffer, tmpdpdiagbuffers, wrkmemaux);
        else
...
// Instantiations
// 
#define INSTANTIATE_ProduceAlignmentUsingDynamicIndex2(tpSECSTRFILT,tpCOMPLEX) \
    template __global__ void ProduceAlignmentUsingDynamicIndex2<tpSECSTRFILT,tpCOMPLEX>( \
        const int stacksize, const bool WRTNDX, const int windowsize, \
        const int seedapproachstruct, const int depth, const float trigger, const uint nqystrs, \
        const uint ndbCstrs, const uint ndbCposs, const uint maxnsteps, \
        const int qryfragfct, const int rfnfragfct, \
        const float* __restrict__ wrkmemtm, \
        float* __restrict__ tmpdpalnpossbuffer, \
...
INSTANTIATE_ProduceAlignmentUsingDynamicIndex2(0,1);
INSTANTIATE_ProduceAlignmentUsingDynamicIndex2(1,0);
INSTANTIATE_ProduceAlignmentUsingDynamicIndex2(1,1);
