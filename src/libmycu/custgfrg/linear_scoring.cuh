...
void NNByIndex(
    ...
    float rx, float ry, float rz, int rt, char rss,
    int qrydst, int root, int dimndx,
    float* __restrict__ stack)
{
    int stackptr = 0;//stack pointer
    ...
    // //NOTE: secondary structure information not used for aligning nucleic acids!
    // //NOTE: implemented elsewhere!
    // if(SECSTRFILT == 1 && isNASS(rss)) rss = 0;
    ...
    while(0 < stackptr || 0 <= root)
    {
        if(0 <= root) {
            ...
            if(CUSF_TBSP_INDEX_SCORE_MAXDEPTH <= nvisited)
                break;
            //map the index in the index tree to the original structure position index
            int /* qt, */ qryorgndx = GetIndxdQueryOrgndx(qrydst + root);
            char qss;
            //NOTE: secondary structure determines type!
            //if(SECSTRFILT == 1) qt = GetMoleculeType(GetQueryStrField<INTYPE,pmv2D_Ins_Ch_Ord>(qrydst + qryorgndx));
            if(SECSTRFILT == 1) qss = GetQuerySS(qrydst + qryorgndx);
            // //NOTE: secondary structure information not used for aligning nucleic acids!
            // //NOTE: implemented elsewhere!
            // if(SECSTRFILT == 1 && isNASS(qss)) qss = 0;
            //READ coordinates:
            float qx = GetIndxdQueryCoord<pmv2DX>(qrydst + root);
            float qy = GetIndxdQueryCoord<pmv2DY>(qrydst + root);
            float qz = GetIndxdQueryCoord<pmv2DZ>(qrydst + root);
            float dst2 = distance2(qx, qy, qz,  rx, ry, rz);
            if((nestndx < 0 || dst2 < nestdst) && //(rt == qt) &&
                //((SECSTRFILT == 1)? !helix_strnd(rss, qss): 1)
                ((SECSTRFILT == 1)? (rss == qss/*  && rt == qt */): 1)
            ) {
                nestdst = dst2;
    ...
    if(SECSTRFILT == 1 && 64.0f < nestdst) nestndx = -1;
}
...
void NNByIndexReference(
    ...
    float qx, float qy, float qz, int qt, char qss,
    ...
    while(0 < stackptr || 0 <= root)
    {
        if(0 <= root) {
            ...
            if(CUSF_TBSP_INDEX_SCORE_MAXDEPTH <= nvisited)
                break;
            //map the index in the index tree to the original structure position index
            int /* rt, */ rfnorgndx = GetIndxdDbStrOrgndx(dbstrdst + root);
            char rss;
            //NOTE: secondary structure determines type!
            //if(SECSTRFILT == 1) rt = GetMoleculeType(GetDbStrField<INTYPE,pmv2D_Ins_Ch_Ord>(dbstrdst + rfnorgndx));
            if(SECSTRFILT == 1) rss = GetDbStrSS(dbstrdst + rfnorgndx);
            // //NOTE: secondary structure information not used for aligning nucleic acids!
            // //NOTE: implemented elsewhere!
            // if(SECSTRFILT == 1 && isNASS(rss)) rss = 0;
            //READ coordinates:
            ...
            float dst2 = distance2(qx, qy, qz,  rx, ry, rz);
            if((nestndx < 0 || dst2 < nestdst) && //(qt == rt) &&
                //((SECSTRFILT == 1)? !helix_strnd(rss, qss): 1)
                ((SECSTRFILT == 1)? (rss == qss/*  && qt == rt */): 1)
            ) {
                nestdst = dst2;
