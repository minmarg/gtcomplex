    ...
    float* __restrict__ wrkmemaux)
{
    ...
    if(COMPLEX) {
        if(threadIdx.x == 0 && threadIdx.y == 0)
            GetDbComplexAddressLength(dbstrndx, (int&)dpsCache[0][1], (int&)dpsCache[0][0]);
        if(threadIdx.x == 0 && threadIdx.y == 2)
            GetQueryComplexAddressLength(qryndx, (int&)dpsCache[0][3], (int&)dpsCache[0][2]);
    } else {
...
// Instantiations
//
#define INSTANTIATE_CalcLocalSimilarity2_frg2(COMPLEX) \
    template __global__ void CalcLocalSimilarity2_frg2<COMPLEX>( \
        const float thrsimilarityperc, const int seedapproachstruct, \
        const int depth, const uint ndbCstrs, const uint ndbCposs, const uint dbxpad, \
        const uint maxnsteps, int qryfragfct, int rfnfragfct, int fragndx, \
        const char* __restrict__ dpscoremtx, \
        float* __restrict__ wrkmemaux);

INSTANTIATE_CalcLocalSimilarity2_frg2(0);
INSTANTIATE_CalcLocalSimilarity2_frg2(1);
