...
void ProduceAlignmentUsingIndex2_Complete(
    ...
    float* __restrict__ tmpdpdiagbuffers)
{
    int fraglen = myhdmin(qrylen, windowsize);
...
// //     dbstrlen = myhdmin(dbstrlen, rfnpos + windowsize);
// //     rfnpos = myhdmax(0, dbstrlen - windowsize);
        ...
        pos0 += blockDim.x)
    {
        ...
        float rx, ry, rz;
        float qx, qy, qz;
        int rt, bestqnx = -1;//query index of the position nearest to a reference atom
