    ...
    float* __restrict__ wrkmem)
{
    ...
    int qrylen, dbstrlen, dummylen;//query and reference length
    ...
    if(COMPLEX) {
        if(threadIdx.x == 0)
            GetDbComplexAddressLength(dbstrndx, ((int*)ccmCache)[1], dummylen);
    }
...
// Instantiations
//
#define INSTANTIATE_CalcCCMatrices64_SWFTscan(tpCOMPLEX) \
    template __global__ void CalcCCMatrices64_SWFTscan<tpCOMPLEX>( \
        const uint nqystrs, const uint ndbCstrs, const uint ndbCposs, \
        const uint /*dbxpad*/, const uint maxnsteps, \
        const float* __restrict__ wrkmemaux, \
        const float* __restrict__ tmpdpalnpossbuffer, \
        float* __restrict__ wrkmem);

INSTANTIATE_CalcCCMatrices64_SWFTscan(0);
INSTANTIATE_CalcCCMatrices64_SWFTscan(1);
...
// CHCKALNLEN, template parameter for checking whether alignment length has 
// changed;
// thrpercentage, threshold percentage of chain occupancy for assignment;
    ...
    float* __restrict__ tmpdpdiagbuffers)
{
    ...
    enum{   pad = 1,//padding
            ...
            xdim = CUS1_TBSP_SCORE_XDIM,//x-dimension of the thread block
            szqnxch = CUSF2_TBSP_INDEX_SCORE_QNX_CACHE_SIZE,//cache size
            nchnsch = CUSF2_TBSP_INDEX_SCORE_COMPLEX_NCHAINS_CACHE_SIZE,//#max complex chains
            nwrpsdim = (xdim>>llog2warpsize)//XDIM/warpsize(2^5)
    };
    __shared__ float SMEM[
        pad + nwrpsdim+1 + szqnxch + szqnxch + xdim + xdim + nTTranformMatrix +
        nchnsch + nchnsch + nchnsch];
    float* tmpSMbuf = SMEM + pad;//size [nwrpsdim+1];
    //cache for query positions: 
    float* qnxCache = tmpSMbuf + nwrpsdim+1;//size [szqnxch]
    //cache for maximum scores over all query positions: 
    float* maxCache = qnxCache + szqnxch;//size [szqnxch]
    //working cache for query positions: 
    float* qnxWrkch = maxCache + szqnxch;//size [xdim]
    //cache for scores and transformation matrix: 
    float* scvCache = qnxWrkch + xdim;//size [xdim]
    //pointer to transformation matrix;
    float* tfmCache = scvCache + xdim;//size [nTTranformMatrix]
    int* qryChains = (int*)(tfmCache + nTTranformMatrix);//query chain boundaries; size [nchnsch]
    int* rfnChains = qryChains + nchnsch;//reference chain boundaries; size [nchnsch]
    int* occChains = rfnChains + nchnsch;//chain occupation flags; size [nchnsch]
    //relative position index:
    const uint ndx0 = blockIdx.x * blockDim.x * xfct;
    const uint ndx = ndx0 + threadIdx.x;
    const uint sfragfct = blockIdx.z / nqystrs;//fragment factor
    ...
    if(threadIdx.x == 0) {
        occChains[0] = GetDbStrField<INTYPE,pcx2DN>(dbstrndx);//#complex chains
        occChains[1] = (int)GetDbStrField<LNTYPE,pcx2DDstN>(dbstrndx);////#chains up to dbstrndx
    }

    if(threadIdx.x == xdim - 1) {
        occChains[2] = GetQueryStrField<INTYPE,pcx2DN>(qryndx);
        occChains[3] = (int)GetQueryStrField<LNTYPE,pcx2DDstN>(qryndx);
    }

    if(threadIdx.x == tawmvNAlnPoss + 32) {
        //NOTE: reuse cache to read #matched positions (tawmvNAlnPoss) written at sfragfct;
        //NOTE: use different warp; structure-specific-formatted data;
        uint mloc = ((qryndx * maxnsteps + sfragfct) * nTAuxWorkingMemoryVars) * ndbCstrs;
        scvCache[threadIdx.x] = wrkmemaux[mloc + (threadIdx.x-32) * ndbCstrs + dbstrndx];
    }
    ...
    int dbcpxN = occChains[0];
    int qrycpxN = occChains[2];
    {//{{read chain addresses and calculate lengths
        int dbcpxDstN = occChains[1];
        int qrycpxDstN = occChains[3];

        for(int ci = threadIdx.x; ci < nchnsch; ci += xdim) {
            rfnChains[ci] = 999999;//initially large
            qryChains[ci] = 999999;//initially large
            if(ci < dbcpxN)
                rfnChains[ci] = (int)GetDbStrField<LNTYPE,pps2DDist>(dbcpxDstN + ci);
            if(ci < qrycpxN)
                qryChains[ci] = (int)GetQueryStrField<LNTYPE,pps2DDist>(qrycpxDstN + ci);
        }

        __syncthreads();

        for(int ci = threadIdx.x+1; ci < nchnsch; ci += xdim) {
            occChains[ci] = 0;//initialize after sync
            //produce running lengths along chains
            if(ci < dbcpxN) rfnChains[ci] -= rfnChains[0];
            if(ci < qrycpxN) qryChains[ci] -= qryChains[0];
        }

        __syncthreads();

        if(threadIdx.x == 0) {
            int dbcpxCN1addr = (int)GetDbStrField<LNTYPE,pps2DDist>(dbcpxDstN + dbcpxN - 1);
            int dbcpxCN1len = GetDbStrField<INTYPE,pps2DLen>(dbcpxDstN + dbcpxN - 1);
            //last chain's length + length excluding the last chain:
            ((int*)scvCache)[0] = dbcpxCN1len + dbcpxCN1addr - rfnChains[0];
            ((int*)scvCache)[1] = rfnChains[0];//address as long as unmodified below
        }

        if(threadIdx.x == xdim - 1) {
            int qrycpxCN1addr = (int)GetQueryStrField<LNTYPE,pps2DDist>(qrycpxDstN + qrycpxN - 1);
            int qrycpxCN1len = GetQueryStrField<INTYPE,pps2DLen>(qrycpxDstN + qrycpxN - 1);
            //last chain's length + length excluding the last chain:
            ((int*)scvCache)[2] = qrycpxCN1len + qrycpxCN1addr - qryChains[0];
        }

        __syncthreads();

        if(threadIdx.x == 0) rfnChains[0] = qryChains[0] = occChains[0] = 0;
    }//}}
    ...
    GetQryRfnPos_frg2(
        depth, qrypos, rfnpos,  qrylenorg, dbstrlenorg,
        sfragfct, qryfragfct, rfnfragfct, (sfragfct & 1)/*fragndx*/,
        seedapproachstruct
    );

    if(qrylenorg < dbstrlenorg) {
        //adjust qrypos:
        int fraglen = myhdmin(dbstrlenorg, windowsize);
        qrypos = myhdmax(0, qrypos - (fraglen>>1));
        int tmpqrylen = myhdmin(qrylenorg, qrypos + fraglen);
        qrypos = myhdmax(0, tmpqrylen - fraglen);
        //NOTE: index-based search in the longer reference structure!
        //NOTE: position indices saved represent reference structure!
        myhdswap(qryChains, rfnChains);
        myhdswap(qrypos, rfnpos);
        myhdswap(qrycpxN, dbcpxN);
    } else {
        //adjust rfnpos:
        int fraglen = myhdmin(qrylenorg, windowsize);
        rfnpos = myhdmax(0, rfnpos - (fraglen>>1));
        int tmpdbstrlen = myhdmin(dbstrlenorg, rfnpos + fraglen);
        rfnpos = myhdmax(0, tmpdbstrlen - fraglen);
    }

    __syncthreads();


    if(qrylen <= ndx0 || dbstrlen <= ndx0)
        //all threads in the block exit if thread 0 is out of bounds
        return;
    ...
    if(CHCKALNLEN == CHCKALNLEN_CHECK) {
        ...
        if(nalnposs == dbstrlen)
            ...
            //all threads in the block exit;
            return;
        //no sync as scvCache[0] is not used below
    }
    ...
    int dbchno = //reference complex chain number
        GetChainNoFromPosition<xdim,nchnsch,nwrpsdim,1/*allthreadssee*/>
            (rfnpos, dbcpxN, rfnChains, tmpSMbuf);
    ...
    for(int i = 0; i < xfct; i++) {
        ...
        int pos00 = ndx0 + i * blockDim.x;//thread-0 position index
        //starting position in tmpdpalnpossbuffer for a pair:
        //NOTE: qrylen == dbstrlen here;
        int dppos = yofff + dbstrdst + (pos0);

        if(dbstrlen <= pos00) break;
        ...
        //process xdim reference positions to progressively find max score
        for(int p = 0; p < xdim && pos00 + p < dbstrlen; p++) {
            float sco = scvCache[p];
            float qnx = qnxWrkch[p];

            if(dbchno+1 < dbcpxN && dbchno+1 < nchnsch && rfnChains[dbchno+1] <= rfnpos) {
                // MakeChainNosPositive<xdim,nchnsch>(qrycpxN, occChains);
                AdjustChainOccupancy<xdim,nchnsch>(thrpercentage, dbchno, qrycpxN, rfnChains, occChains);
                dbchno++;
            }
            ...
            int qrychno = //query complex chain number
                GetChainNoFromPosition<xdim,nchnsch,nwrpsdim,1/*allthreadssee*/>
                    ((int)qnx, qrycpxN, qryChains, tmpSMbuf);

            if(0 < occChains[qrychno]) continue;

            //find max score up to position qnx:
            float max =
                FindMax_SWFTProgressive<0/*pad*/,xdim,szqnxch,nwrpsdim,false/*uncnd*/>(
                    qnx, qnxCache, maxCache, tmpSMbuf);

            //save max score to cache:
            if(threadIdx.x == 0) {
                //increment the chain occupancy flag:
                occChains[qrychno] -= 1;
                //extremely simple hash function for the cache index:
                int c = (int)(qnx) & (szqnxch-1);
                ...
                float newsco = max + sco;//new score
                bool half2nd = (pos00 + p > (dbstrlen>>1));
                //heuristics: under hash collision, update position and 
                //score wrt to which reference half is under process:
                if(stqnx < 0.0f ||
                  (stqnx == qnx && stsco < newsco) ||
    ...
    //find max score over all query positions:
    float max =
        FindMax_SWFTProgressive<0/*pad*/,xdim,szqnxch,nwrpsdim,true/*uncnd*/>(
            0.0f, qnxCache, maxCache, tmpSMbuf);
...
// Instantiations
// 
#define INSTANTIATE_CalcScoresUnrl_SWFTscanProgressiveComplex(tpSAVEPOS,tpCHCKALNLEN) \
    template __global__ void CalcScoresUnrl_SWFTscanProgressiveComplex<tpSAVEPOS,tpCHCKALNLEN>( \
        const float thrpercentage_chainoccupancy, const int windowsize, \
        const int seedapproachstruct, const int depth, const uint nqystrs, const uint ndbCstrs, \
        const uint ndbCposs, const uint dbxpad, \
        const uint maxnsteps, const int qryfragfct, const int rfnfragfct, \
        const float* __restrict__ tmpdpalnpossbuffer, \
        const float* __restrict__ wrkmemtm, \
        const float* __restrict__ wrkmem, \
        float* __restrict__ wrkmemaux, \
...
INSTANTIATE_CalcScoresUnrl_SWFTscanProgressiveComplex(SAVEPOS_SAVE,CHCKALNLEN_NOCHECK);
INSTANTIATE_CalcScoresUnrl_SWFTscanProgressiveComplex(SAVEPOS_SAVE,CHCKALNLEN_CHECK);
INSTANTIATE_CalcScoresUnrl_SWFTscanProgressiveComplex(SAVEPOS_NOSAVE,CHCKALNLEN_CHECK);
INSTANTIATE_CalcScoresUnrl_SWFTscanProgressiveComplex(SAVEPOS_NOSAVE,CHCKALNLEN_NOCHECK);
