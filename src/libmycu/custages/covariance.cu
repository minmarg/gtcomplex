...
// -------------------------------------------------------------------------
// CalcCCMatrices64LocallyAligned: calculate cross-covariance matrix between
...
__global__
void CalcCCMatrices64LocallyAligned(
    ...
    float* __restrict__ wrkmem)
{
    enum {
        ...
        QX = 0, QY = 1, QZ = 2,  RX = 3, RY = 4, RZ = 5,
        NSCORESCHECK = 4,
        log2NSCORESCHECK = 2,
        SMIDIM = twmvEndOfCCDataExt,
        lXDIM = CUSF_TBSP_INITIAL_ALN_CCM_XDIM,
        lYDIM = CUSF_TBSP_INITIAL_ALN_CCM_YDIM
    };
    ...
    __shared__ int posCache[bmTotal][lXDIM+1];//match positions
    __shared__ float crdCache[lYDIM][lXDIM+1];//coordinates
    ...
    if(COMPLEX) {
        ...
        if(threadIdx.y == 0 && threadIdx.x == 0) GetDbStrLenDst(dbstrndx, (int*)ccmCache);
        if(threadIdx.y == 1 && threadIdx.x == 0) GetQueryLenDst(qryndx, (int*)ccmCache + 2);
    }
    ...
    if(PositionsOutofBounds_frg(qrylen, dbstrlen, qrypos, rfnpos, qryfragfct, rfnfragfct, 0/*fragndx; use smaller*/,
            seedapproachstruct))
        //all threads in the block exit
        return;
    ...
    if(threadIdx.y == 0 && threadIdx.x < NSCORESCHECK) {
    ...
    for(int c = (NSCORESCHECK>>1); c >= 1; c >>= 1) {
        if(threadIdx.y == 0 && threadIdx.x < c && ccmCache[threadIdx.x] < ccmCache[threadIdx.x + c]) {
    ...
    int y = ccmCache[1 * NSCORESCHECK];//query end position
    int x = ccmCache[2 * NSCORESCHECK];//reference end position
    __syncthreads();


    //initialize cache; not read until the next sync;
    if(threadIdx.y == 0 && threadIdx.x == 0) posCache[bmQRYNDX][lXDIM] = 0;
    if(threadIdx.y == 1 && threadIdx.x == 0) posCache[bmRFNNDX][lXDIM] = dpbtckSTOP;

    for(int i = threadIdx.y; i < SMIDIM; i += blockDim.y)
    ...
    while(btck != dpbtckSTOP) {
        ...
        if(threadIdx.x < ndx) {
            int ypos/*qrypos*/ = qrydst + posCache[bmQRYNDX][threadIdx.x];
            int xpos/*rfnpos*/ = dbstrdst + posCache[bmRFNNDX][threadIdx.x];

            if(threadIdx.y == QX) crdCache[QX][threadIdx.x] = GetQueryCoord<pmv2DX>(ypos);
            if(threadIdx.y == QY) crdCache[QY][threadIdx.x] = GetQueryCoord<pmv2DY>(ypos);
            if(threadIdx.y == QZ) crdCache[QZ][threadIdx.x] = GetQueryCoord<pmv2DZ>(ypos);

            if(threadIdx.y == RX) crdCache[RX][threadIdx.x] = GetDbStrCoord<pmv2DX>(xpos);
            if(threadIdx.y == RY) crdCache[RY][threadIdx.x] = GetDbStrCoord<pmv2DY>(xpos);
            if(threadIdx.y == RZ) crdCache[RZ][threadIdx.x] = GetDbStrCoord<pmv2DZ>(xpos);
        }
        __syncthreads();
        if(threadIdx.x < ndx) {
            float qx = crdCache[QX][threadIdx.x], rx = crdCache[RX][threadIdx.x];
            float qy = crdCache[QY][threadIdx.x], ry = crdCache[RY][threadIdx.x];
            float qz = crdCache[QZ][threadIdx.x], rz = crdCache[RZ][threadIdx.x];

            if(threadIdx.y == 0) ccmCache[threadIdx.x * SMIDIM + twmvCCM_0_0] += qx * rx;
            if(threadIdx.y == 1) ccmCache[threadIdx.x * SMIDIM + twmvCCM_0_1] += qx * ry;
            if(threadIdx.y == 2) ccmCache[threadIdx.x * SMIDIM + twmvCCM_0_2] += qx * rz;

            if(threadIdx.y == 3) ccmCache[threadIdx.x * SMIDIM + twmvCCM_1_0] += qy * rx;
            if(threadIdx.y == 4) ccmCache[threadIdx.x * SMIDIM + twmvCCM_1_1] += qy * ry;
            if(threadIdx.y == 5) ccmCache[threadIdx.x * SMIDIM + twmvCCM_1_2] += qy * rz;

            if(threadIdx.y == 0) ccmCache[threadIdx.x * SMIDIM + twmvCCM_2_0] += qz * rx;
            if(threadIdx.y == 1) ccmCache[threadIdx.x * SMIDIM + twmvCCM_2_1] += qz * ry;
            if(threadIdx.y == 2) ccmCache[threadIdx.x * SMIDIM + twmvCCM_2_2] += qz * rz;

            if(threadIdx.y == 3) ccmCache[threadIdx.x * SMIDIM + twmvCVq_0] += qx;
            if(threadIdx.y == 4) ccmCache[threadIdx.x * SMIDIM + twmvCVq_1] += qy;
            if(threadIdx.y == 5) ccmCache[threadIdx.x * SMIDIM + twmvCVq_2] += qz;

            if(threadIdx.y == 0) ccmCache[threadIdx.x * SMIDIM + twmvCVr_0] += rx;
            if(threadIdx.y == 1) ccmCache[threadIdx.x * SMIDIM + twmvCVr_1] += ry;
            if(threadIdx.y == 2) ccmCache[threadIdx.x * SMIDIM + twmvCVr_2] += rz;

            if(threadIdx.y == 3) ccmCache[threadIdx.x * SMIDIM + twmvNalnposs] += 1.0f;
        }
        __syncthreads();
    ...
    if(alnlen < 1 || (trigger && alnlen < myhdmin(16, fraglen))) {
        uint mloc = ((qryndx * maxnsteps + sfragfct) * nTAuxWorkingMemoryVars + tawmvConverged) * ndbCstrs;
        if(threadIdx.y == 0 && threadIdx.x == 0 && sfragfct) wrkmemaux[mloc + dbstrndx] = CONVERGED_SCOREDP_bitval;
        return;//all exit
    }


    //4. final reduction
    //NOTE: synced above; unroll within warps
    for(int i = threadIdx.y; i < SMIDIM; i += blockDim.y) {
    ...
    if(threadIdx.y == 0 && threadIdx.x < SMIDIM) {
...
// Instantiations
//
#define INSTANTIATE_CalcCCMatrices64LocallyAligned(COMPLEX) \
    template __global__ void CalcCCMatrices64LocallyAligned<COMPLEX>( \
        const int seedapproachstruct, const int depth, const float trigger, \
        const uint ndbCstrs, const uint ndbCposs, const uint dbxpad, \
        const uint maxnsteps, int qryfragfct, int rfnfragfct, \
        const char* __restrict__ dpscoremtx, \
        float* __restrict__ wrkmemaux, float* __restrict__ wrkmem);

INSTANTIATE_CalcCCMatrices64LocallyAligned(1);
INSTANTIATE_CalcCCMatrices64LocallyAligned(0);
    ...
    float* __restrict__ wrkmem)
{
    ...
    if(COMPLEX) {
        ...
        {
            GetQueryComplexAddressLength(qryndx, ((int*)ccmCache)[3], ((int*)ccmCache)[2]);
            ((int*)ccmCache)[4] = GetQueryStrField<INTYPE,pcx2DType>(qryndx);
        }
    }
    ...
    if(COMPLEX) qrytype = GetMoleculeType(((int*)ccmCache)[4]);
    ...
    if(threadIdx.x < NSCORESCHECK) {
        ...
        vr = dpscoremtx[rpos] & 0xfc;//two least significant bits for backtracking!
    }
    for(int c = (NSCORESCHECK>>1); c >= 1; c >>= 1) {
...
// Instantiations
//
#define INSTANTIATE_CalcCCMatrices64LocallyAligned64(COMPLEX) \
    template __global__ void CalcCCMatrices64LocallyAligned64<COMPLEX>( \
        const int seedapproachstruct, const int depth, const float trigger, \
        const uint ndbCstrs, const uint ndbCposs, const uint dbxpad, \
        const uint maxnsteps, int qryfragfct, int rfnfragfct, \
        const char* __restrict__ dpscoremtx, \
...
INSTANTIATE_CalcCCMatrices64LocallyAligned64(1);
INSTANTIATE_CalcCCMatrices64LocallyAligned64(0);
    ...
    float* __restrict__ wrkmem)
{
    ...
    CalcCCMatrices64Helper<0/*COMPLEX*/,CHCKCONV_NOCHECK>(
        GetQryRfnPos, PositionsOutofBounds, 0/*depth; unused*/,
        sfragfct, qryndx, ndbCstrs, maxnsteps,  n1, step, 0/*arg3(unused)*/,
        wrkmemaux, wrkmem);
}
    ...
    float* __restrict__ wrkmem)
{
    ...
    CalcCCMatrices64Helper<COMPLEX,CHCKCONV_CHECK>(
        GetQryRfnPos_frg2, PositionsOutofBounds_frg, depth,
        sfragfct, qryndx, ndbCstrs, maxnsteps,  qryfragfct, rfnfragfct, fragndx,
        wrkmemaux, wrkmem);
}
...
// Instantiations
//
#define INSTANTIATE_CalcCCMatrices64_frg2(COMPLEX) \
    template __global__ void CalcCCMatrices64_frg2<COMPLEX>( \
        const int depth, const uint nqystrs, const uint ndbCstrs, \
        const uint maxnsteps, int qryfragfct, int rfnfragfct, int fragndx, \
        const float* __restrict__ wrkmemaux, \
        float* __restrict__ wrkmem);

INSTANTIATE_CalcCCMatrices64_frg2(0);
INSTANTIATE_CalcCCMatrices64_frg2(1);
    ...
    const int seedapproachstruct = 0)
{
    ...
    if(absndx < ndbCstrs && threadIdx.x == 0 && threadIdx.y == 0) {
        ...
        if(COMPLEX) {
            int address, length;
            GetQueryComplexAddressLength(qryndx, address, length);
            ccmCache[0][0] = length;
        } else
    ...
    if(absndx < ndbCstrs && threadIdx.y == 0) {
        if(CHCKCONV == CHCKCONV_CHECK && ccmCache[threadIdx.x][tawmvConverged]) {
            ...
            int qrypos, rfnpos, nlposs, dummyaddr;
            if(COMPLEX) GetDbComplexAddressLength(absndx, dummyaddr, dbstrlen);
            else dbstrlen = GetDbStrLength(absndx);
    ...
    float* __restrict__ wrkmem2)
{
    ...
    CopyCCDataToWrkMem2Helper<0/*COMPLEX*/,CHCKCONV_NOCHECK,READNPOS>(
...
// same as CopyCCDataToWrkMem2_var with more extensive parallelization;
// depth, superposition depth for calculating query and reference positions;
template<int READNPOS, int COMPLEX>
__global__
    ...
    float* __restrict__ wrkmem2)
{
    ...
    CopyCCDataToWrkMem2Helper<COMPLEX,CHCKCONV_CHECK,READNPOS>(
...
// Instantiations
//
#define INSTANTIATE_CopyCCDataToWrkMem2_frg2(READNPOS,COMPLEX) \
    template __global__ void CopyCCDataToWrkMem2_frg2<READNPOS,COMPLEX>( \
...
INSTANTIATE_CopyCCDataToWrkMem2_frg2(READNPOS_NOREAD,0);
INSTANTIATE_CopyCCDataToWrkMem2_frg2(READNPOS_NOREAD,1);
INSTANTIATE_CopyCCDataToWrkMem2_frg2(READNPOS_READ,0);
INSTANTIATE_CopyCCDataToWrkMem2_frg2(READNPOS_READ,1);
