    ...
    char* __restrict__ dpscoremtx)
{
    ...
    if(COMPLEX) {
        if(threadIdx.x == 0)
            GetDbComplexAddressLength(dbstrndx, ((int*)diag2Cache)[1], ((int*)diag2Cache)[0]);

        if((CUDP_2DCACHE_DIM_D <= lWarpsize && threadIdx.x == 1) ||
           (CUDP_2DCACHE_DIM_D  > lWarpsize && threadIdx.x == lWarpsize))
        {
            GetQueryComplexAddressLength(qryndx, ((int*)diag2Cache)[3], ((int*)diag2Cache)[2]);
            ((int*)diag2Cache)[4] = GetQueryStrField<INTYPE,pcx2DType>(qryndx);
        }
    }
    ...
    if(COMPLEX) qrytype = GetMoleculeType(((int*)diag2Cache)[4]);
    ...
    if(0 <= qpos && qpos < qrylen) {
        if(qrytype == gtmtProtein) DPLocCacheQrySS(qrySS, qpos + qrydst);
    ...
    if(0 <= x && x < dbstrlen) {
        if(qrytype == gtmtProtein) DPLocCacheRfnSS<0/*shift*/>(rfnSS, dbpos);
    ...
    if(0 <= (x+CUDP_2DCACHE_DIM_D) && (x+CUDP_2DCACHE_DIM_D) < dbstrlen) {
        ...
        if(qrytype == gtmtProtein) DPLocCacheRfnSS<CUDP_2DCACHE_DIM_D>(rfnSS, dbpos + CUDP_2DCACHE_DIM_D);
...
// Instantiations
//
#define INSTANTIATE_ExecDPSSLocal3264x(tpCOMPLEX) \
    template __global__ void ExecDPSSLocal3264x<tpCOMPLEX>( \
        const uint blkdiagnum, \
...
INSTANTIATE_ExecDPSSLocal3264x(1);
