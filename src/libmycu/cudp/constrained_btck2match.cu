    ...
    const uint assgstepnumber)
{
    ...
    if(COMPLEX) {
        ...
        __syncthreads();
        //NOTE: posCache[0][4-5] cannot be overwritten until the next sync!
    ...
    if(threadIdx.x == 0 && threadIdx.y == 0) {
        ...
        posCache[1][0] = wrkmemaux[mloc + tawmvConverged * ndbCstrs + dbcpxndx];//float->int
    }
    ...
    if(COMPLEX) {
        int addr;
        if(threadIdx.x == 0 && threadIdx.y == 1) GetDbComplexAddressLength(dbcpxndx, addr, posCache[0][4]);
        if(threadIdx.x == 0 && threadIdx.y == 2) GetQueryComplexAddressLength(qrycpxndx, addr, posCache[0][5]);
    }
    ...
    if(threadIdx.x == 0 && threadIdx.y == 1) {
        ...
        posCache[0][6] = GetDbStrField<INTYPE,pmv2D_Ins_Ch_Ord>(dbstrdst);
        posCache[0][7] = GetQueryStrField<INTYPE,pmv2D_Ins_Ch_Ord>(qrydst);
    }

    __syncthreads();

    // if(GetMoleculeType(posCache[0][6]) != GetMoleculeType(posCache[0][7]))
    //     return;//NOTE: no write to [4,5] until the next sync!

    if(COMPLEX &&
       ChainsWithinAssignment<CUDP_CONST_MATCH_DIM_X/*xdim*/,CUDP_CONST_MATCH_DIM_Y/*ydim*/>(
    ...
    dbcpxlen = dbstrlen = posCache[0][0]; dbstrdst = posCache[0][1];
    qrycpxlen = qrylen = posCache[0][2]; qrydst = posCache[0][3];

    if(COMPLEX) {
        dbcpxlen = posCache[0][4];
        qrycpxlen = posCache[0][5];
    }

    if(threadIdx.x < nTTranformMatrix && threadIdx.y == 0) {
        //globally best transformation matrix for a pair:
        uint mloc0 = (qrycpxndx/*qryndx*/ * ndbCstrs + dbcpxndx/*dbstrndx*/) * nTTranformMatrix;
    ...
    const int yoff1 = (qrycpxndx * maxnsteps + complexstepnumber2/*sfragfct*/) * dblen * nTDPAlignedPoss;
    const float d82 = GetD82(qrycpxlen, dbcpxlen);
...
// Instantiations
//
#define INSTANTIATE_ConstrainedBtckToMatched32x(tpCOMPLEX) \
    template \
    __global__ void ConstrainedBtckToMatched32x<tpCOMPLEX>( \
...
INSTANTIATE_ConstrainedBtckToMatched32x(false);
INSTANTIATE_ConstrainedBtckToMatched32x(true);
// -------------------------------------------------------------------------
