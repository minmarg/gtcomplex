...
// CUDP_2DCACHE_DIM;
// this version fills in the dp matrix with scores w/o backtracking
// information;
...
// #endif
    ...
    for(int i = 0; i < ilim/*CUDP_2DCACHE_DIM_X*/; i++)
    {
        ...
        val1 = (float)(qrySS == rfnSS[threadIdx.x+i]);
        ...
        val1 += fabsf(pdiag2[GetBufferNdx<DDIM>(dpdsssStateMM,threadIdx.x+1)]);
        btck = dpbtckDIAG;
        ...
        if(val2 < 0.0f) val2 = gapopencost - val2;
...
#ifdef CUDPSS_TESTPRINT
        if(dbstrndx==CUDPSS_TESTPRINT){
