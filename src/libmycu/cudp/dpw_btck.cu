...
// NOTE: ALTSCTMS, template flag indicating the use of alternative memory section of tms;
// NOTE: COMPLEX, template parameter, flag of transformation matrices obtained for full complexes;
// NOTE: MAX_NCHAINS, max #complex chains for this kernel to continue computation;
    ...
    // const uint ndbCcpxs)
{
    ...
    enum {  DDIM = CUDP_2DCACHE_DIM_D + 1,//inner dimension for diagonal buffers
            nwrpsdim = (DDIM>>llog2warpsize)//DDIM/warpsize(2^5)
    };
...
//                         pmv2DNoElems * (CUDP_2DCACHE_DIM_D + CUDP_2DCACHE_DIM_DpX);
    ...
    int qrycpxlen, dbcpxlen;//complex lengths
    ...
    if((COMPLEX || MAX_NCHAINS > 0) && threadIdx.x == 0) {
        int lqrycpxndx = GetQueryStrField<INTYPE,pps2DCpxI>(qryndx);
        int ldbcpxndx = GetDbStrField<INTYPE,pps2DCpxI>(dbstrndx);
        if(COMPLEX) {
            tfmCache[4] = lqrycpxndx;//float<-int
            tfmCache[5] = ldbcpxndx;//float<-int 
        }
        if(MAX_NCHAINS > 0) {
            int qrycpxN = GetQueryStrField<INTYPE,pcx2DN>(lqrycpxndx);//#chains
            int dbcpxN = GetDbStrField<INTYPE,pcx2DN>(ldbcpxndx);//#chains
            tfmCache[6] = (MAX_NCHAINS < qrycpxN) && (MAX_NCHAINS < dbcpxN);
        }
    }
    ...
    //NOTE: tfmCache[6] cannot be overwritten until the next sync!
    if(MAX_NCHAINS > 0 && tfmCache[6]) return;//block exits

    if(COMPLEX) {
        qrycpxndx = (uint)tfmCache[4];
        dbcpxndx = (uint)tfmCache[5];//uint<-float
    }
    ...
    if(threadIdx.x == 0) {
        ...
        rfnCoords[0] = wrkmemaux[mloc0 + dbcpxndx];
    ...
    if(stepnumber != 0 &&
        ...
        rfnCoords[1] = wrkmemaux[mloc + dbcpxndx];
    }
    ...
    if(threadIdx.x == 0) {
        ...
        ((int*)tfmCache)[2] = GetQueryLength(qryndx);
        ((int*)tfmCache)[3] = qrydst = GetQueryDst(qryndx);
        ((int*)tfmCache)[4] = GetDbStrField<INTYPE,pmv2D_Ins_Ch_Ord>(dbstrdst);
        ((int*)tfmCache)[5] = GetQueryStrField<INTYPE,pmv2D_Ins_Ch_Ord>(qrydst);
    }
    ...
    if(GetMoleculeType(((int*)tfmCache)[4]) != GetMoleculeType(((int*)tfmCache)[5]))
    ...
    dbcpxlen = dbstrlen = ((int*)tfmCache)[0]; dbstrdst = ((int*)tfmCache)[1];
    qrycpxlen = qrylen = ((int*)tfmCache)[2]; qrydst = ((int*)tfmCache)[3];

    if(COMPLEX && wrkmem2 && assgmaxnsteps &&
       ChainsWithinAssignment<CUDP_2DCACHE_DIM_D/*xdim*/,1/*ydim*/>(
            qrycpxndx, dbcpxndx,  qryndx, dbstrndx,  ndbCstrs,
            assgmaxnsteps, assgstepnumber,
            wrkmem2, bottmCache) == 0)
        //NOTE: bottmCache should not be overwritten until the next sync!
        //all threads exit if chains are not mutually assigned;
        return;

    if(COMPLEX) {
        int addr;
        if(threadIdx.x == 0) GetDbComplexAddressLength(dbcpxndx, addr, ((int*)tfmCache)[6]);
#if (CUDP_2DCACHE_DIM_D <= 32)
        if(threadIdx.x == 0)
#else
        if(threadIdx.x == 32)
#endif
            GetQueryComplexAddressLength(qrycpxndx, addr, ((int*)tfmCache)[8]);
    }
    ...
    if(COMPLEX) {
        dbcpxlen = ((int*)tfmCache)[6];
        qrycpxlen = ((int*)tfmCache)[8];
#if (CUDP_2DCACHE_DIM_D <= 32)
        __syncwarp();
#else
        __syncthreads();
    ...
    if(threadIdx.x < nTTranformMatrix) {
        ...
        uint mloc0 = ((qrycpxndx/*qryndx*/ * maxnsteps + 0) * ndbCstrs + dbcpxndx/*dbstrndx*/) * nTTranformMatrix;
        ...
        if(ALTSCTMS)
            mloc0 = ((qrycpxndx/*qryndx*/ * CUS1_TBSP_DPSCORE_TOP_N_REFINEMENTxMAX_CONFIGS + stepnumber) *
                ndbCstrs + dbcpxndx/*dbstrndx*/) * nTTranformMatrix;
...
// #endif
    ...
    float d02;
    if(D02IND == D02IND_SEARCH) d02 = GetD02(qrycpxlen, dbcpxlen);
    else if(D02IND == D02IND_DPSCAN) d02 = GetD02_dpscan(qrycpxlen, dbcpxlen);
...
// #endif
    ...
    //NOTE: introduced for complex chain assignment;
...
// Instantiations
//
#define INSTANTIATE_ExecDPwBtck3264x( \
        tpANCHORRGN,tpBANDED,tpGAP0,tpD02IND,tpALTSCTMS,tpCOMPLEX,tpMAX_NCHAINS) \
    template __global__ void ExecDPwBtck3264x \
        <tpANCHORRGN,tpBANDED,tpGAP0,tpD02IND,tpALTSCTMS,tpCOMPLEX,tpMAX_NCHAINS>( \
            const uint blkdiagnum, \
...
INSTANTIATE_ExecDPwBtck3264x(false,false,false,D02IND_SEARCH,false,false,0);
INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_SEARCH,false,false,0);
INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_SEARCH,true,false,0);
// INSTANTIATE_ExecDPwBtck3264x(true,false,false,D02IND_SEARCH,false,false);
// INSTANTIATE_ExecDPwBtck3264x(true,false,true,D02IND_SEARCH,false,false);
// INSTANTIATE_ExecDPwBtck3264x(true,true,false,D02IND_SEARCH,false,false);
// INSTANTIATE_ExecDPwBtck3264x(true,true,true,D02IND_SEARCH,false,false);

INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_DPSCAN,false,false,0);
// INSTANTIATE_ExecDPwBtck3264x(true,false,true,D02IND_DPSCAN,false,false);
// INSTANTIATE_ExecDPwBtck3264x(true,true,true,D02IND_DPSCAN,false,false);


INSTANTIATE_ExecDPwBtck3264x(false,false,false,D02IND_SEARCH,false,true,0);
INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_SEARCH,true,true,0);
INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_SEARCH,false,true,0);


INSTANTIATE_ExecDPwBtck3264x(false,false,false,D02IND_SEARCH,false,false,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_SEARCH,false,false,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_SEARCH,false,true,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
INSTANTIATE_ExecDPwBtck3264x(false,false,true,D02IND_SEARCH,true,true,CUS1_TBSP_CPXSCORE_MAX_NCHAINS);
