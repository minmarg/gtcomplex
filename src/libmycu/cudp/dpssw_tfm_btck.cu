...
// NOTE: COMPLEX, template parameter, flag of aligning complexes;
    ...
    const uint assgstepnumber)
{
    ...
    if(threadIdx.x == 0) {
        uint mloc = ((qrycpxndx * maxnsteps + 0/*sfragfct*/) * nTAuxWorkingMemoryVars) * ndbCstrs;
        rfnCoords[0] = wrkmemaux[mloc + tawmvConverged * ndbCstrs + dbcpxndx];
    }
    ...
    if(threadIdx.x < nTTranformMatrix) {
        ...
        if(GLOBTFM) {
            ...
            mloc0 = (qrycpxndx/*qryndx*/ * ndbCstrs + dbcpxndx/*dbstrndx*/) * nTTranformMatrix;
        } else {
            ...
            mloc0 = ((qrycpxndx/*qryndx*/ * maxnsteps + stepnumber) *
...
// Instantiations
//
#define INSTANTIATE_ExecDPTFMSSwBtck3264x(tpGLOBTFM,tpGAP0,tpUSESS,tpD02IND,tpCOMPLEX) \
    template \
    __global__ void ExecDPTFMSSwBtck3264x<tpGLOBTFM,tpGAP0,tpUSESS,tpD02IND,tpCOMPLEX>( \
        const uint blkdiagnum, \
        ...
        const float* __restrict__ tfmmem, \
        const float* __restrict__ /* wrkmemaux */, \
        float* __restrict__ tmpdpdiagbuffers, float* __restrict__ tmpdpbotbuffer, \
        char* __restrict__ btckdata, float* __restrict__ wrkmem2, \
        const uint assgmaxnsteps, const uint assgstepnumber);

INSTANTIATE_ExecDPTFMSSwBtck3264x(true,false,true,D02IND_DPSCAN,false);
INSTANTIATE_ExecDPTFMSSwBtck3264x(true,true,false,D02IND_SEARCH,false);

INSTANTIATE_ExecDPTFMSSwBtck3264x(true,false,true,D02IND_DPSCAN,true);
INSTANTIATE_ExecDPTFMSSwBtck3264x(true,true,false,D02IND_SEARCH,true);
// -------------------------------------------------------------------------
