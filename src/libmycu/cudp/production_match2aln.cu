    ...
    const uint assgstepnumber)
{
    ...
    enum {
        blockDim_y = CUDP_PRODUCTION_ALN_DIM_Y,//y-dimension
    ...
    if(COMPLEX && threadIdx.x == 0 && threadIdx.y == 0) {
        outAlnCache[4] = GetQueryStrField<INTYPE,pps2DCpxI>(qryndx);
        outAlnCache[5] = GetDbStrField<INTYPE,pps2DCpxI>(dbstrndx);
    }

    if(COMPLEX) {
        __syncthreads();
        qrycpxndx = outAlnCache[4];
        dbcpxndx = outAlnCache[5];
    }

    //check convergence
    if(threadIdx.x == 0 && threadIdx.y == 0) {
        //check for coonvergence at sfragfct==0:
        uint mloc = ((qrycpxndx * maxnsteps + 0) * nTAuxWorkingMemoryVars) * ndbCstrs;
        outAlnCache[31] = wrkmemaux[mloc + tawmvConverged * ndbCstrs + dbcpxndx];
    }
    ...
    if(threadIdx.y == 0) {
        ...
        if((threadIdx.x == dp2oadOrgStrNo) || 
           (nTDP2OutputAlnDataPart1Beg <= threadIdx.x && threadIdx.x < nTDP2OutputAlnDataPart1End)) {
...
// Instantiations
//
#define INSTANTIATE_ProductionMatchToAlignment32x(tpCOMPLEX) \
    template \
    __global__ void ProductionMatchToAlignment32x<tpCOMPLEX>( \
        const bool nodeletions, const float d2equiv, \
        const uint ndbCstrs, const uint ndbCposs, const uint dbxpad, \
        const uint maxnsteps, \
        const float* __restrict__ tmpdpalnpossbuffer, const float* __restrict__ wrkmemaux, \
        float* __restrict__ alndatamem, char* __restrict__ alnsmem, float* __restrict__ wrkmem2, \
        const uint complexstepnumber, const uint complexstepnumber2, \
        const uint assgmaxnsteps, const uint assgstepnumber);

INSTANTIATE_ProductionMatchToAlignment32x(false);
INSTANTIATE_ProductionMatchToAlignment32x(true);
// -------------------------------------------------------------------------
