...
// maxnstepschains, maxnsteps version for temporary buffers (tmpdpdiagbuffers, tmpdpbotbuffer);
// 
template<bool ANCHORRGN, bool BANDED, bool GAP0, bool CHECKCONV, bool COMPLEX>
__global__
    ...
    const uint maxnstepschains)
{
    ...
    enum {  DDIM = CUDP_SWFT_2DCACHE_DIM_D + 1,//inner dimension for diagonal buffers
    ...
    const uint qryndxpart = blockIdx.z - sfragfct * nqystrs;//query partial serial number
    ...
    if(COMPLEX && threadIdx.x == 0) {
        tfmCache[4] = GetQueryStrField<INTYPE,pps2DCpxI>(qryndx);//float<-int
        tfmCache[5] = GetDbStrField<INTYPE,pps2DCpxI>(dbstrndx);//float<-int
    }
...
#endif
            uint mloc = ((qrycpxndx * maxnsteps + sfragfct) * nTAuxWorkingMemoryVars + tawmvConverged) * ndbCstrs;
    ...
    if(COMPLEX) {
        int addr;
#if (CUDP_SWFT_2DCACHE_DIM_D <= 64)
        if(threadIdx.x == 0)
#else
        if(threadIdx.x == 32)
#endif
            GetDbComplexAddressLength(dbcpxndx, addr, ((int*)tfmCache)[6]);
#if (CUDP_SWFT_2DCACHE_DIM_D <= 32)
        if(threadIdx.x == 0)
#elif (CUDP_SWFT_2DCACHE_DIM_D <= 64)
        if(threadIdx.x == 32)
#else
        if(threadIdx.x == 64)
#endif
    ...
    if(threadIdx.x == 0) {
        ...
        ((int*)tfmCache)[10] = GetDbStrField<INTYPE,pmv2D_Ins_Ch_Ord>(dbstrdst);
        ((int*)tfmCache)[11] = GetQueryStrField<INTYPE,pmv2D_Ins_Ch_Ord>(qrydst);
    }
    ...
    if(GetMoleculeType(((int*)tfmCache)[10]) != GetMoleculeType(((int*)tfmCache)[11]))
    ...
    int yofff = (qryndxpart * (maxnstepschains? maxnstepschains: maxnsteps) + sfragfct) * dblen;
    ...
    if(threadIdx.x < nTTranformMatrix) {
        ...
        uint mloc = ((qrycpxndx/*qryndx*/ * maxnsteps + /*0*/sfragfct) * ndbCstrs + dbcpxndx/*dbstrndx*/) * nTTranformMatrix;
    ...
    float d02 = GetD02/*_dpscan*/(qrycpxlen, dbcpxlen);
...
// Instantiations
//
#define INSTANTIATE_ExecDPScore3264x(tpANCHORRGN,tpBANDED,tpGAP0,tpCHECKCONV,tpCOMPLEX) \
    template \
    __global__ void ExecDPScore3264x<tpANCHORRGN,tpBANDED,tpGAP0,tpCHECKCONV,tpCOMPLEX>( \
        const uint blkdiagnum, \
        ...
        float* __restrict__ tmpdpdiagbuffers, \
        float* __restrict__ tmpdpbotbuffer, \
        const uint nqystrstartindex, const uint maxnstepschains);

INSTANTIATE_ExecDPScore3264x(false,false,true,true,false);
INSTANTIATE_ExecDPScore3264x(false,false,true,true,true);
INSTANTIATE_ExecDPScore3264x(false,false,true,false,true);
