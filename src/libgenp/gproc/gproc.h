...
#define COMBINE_WORDS(WH, WL) ( (((WH) & 0xffff) << 16) | ((WL) & 0xffff) )
#define GET_HWORD(DW) ( ((DW) >> 16) & 0xffff )
#define GET_LWORD(DW) ( (DW) & 0xffff )

// Distance of equivalence for aligned residues
#define EQUIVALENCE_DISTANCE (5.0f)

// === COMPLEX template parameter values ===================================

enum {
    ctpvNO_COMPLEX,//processing single chains independently
    ctpvCOMPLEX,//processing complexes
    ctpvCOMPLEX_CS//processing chains within the context of complexes
};
...
enum TAuxWorkingMemoryVars {
    ...
    tawmvInitialBest,//unrefined initial best score; also, chain indices for complex chain assignment 
...
// supporting step numbers corresponding to memory sections for 
// complex processing 
enum TComplexSupportingStepNumbers {
    chor_cpx_complexstepnumber = 1,//step number for keeping chains-specific #aligned residues
    frg3_cpx_complexstepnumber = 1,//--"--
    stg1_cpx_complexstepnumber = 1,//--"--
    ssrr_cpx_complexstepnumber = 2,//--"--
    stg2_cpx_complexstepnumber = 2,//--"--
    sfin_cpx_complexstepnumber = 2,//--"--
    sfin_cpx_complexstepnumber2 = 3,//positions of chains-specific #aligned residues
    sfin_cpx_complextypenumber2 = 4//step number for keeping aligned complex type
};
...
// indices for chain assignment variant:
enum TWorkingMemory2VarsAssignment {
    twm2aChnScore,//chain-chain score
    twm2aCpxScore,//complex score originating from chain assignment
    twm2aCCAssignment,//chain-to-chain assignment
    twm2aTmpConvergence = twm2aCCAssignment,//temporarily used for convergence
    nTWorkingMemory2VarsAssignment
};
...
enum TDP2OutputAlnData {
    ...
    dp2oadD0Q,//Normalizing inter-residue distance d0 for query
    dp2oadD0R,//Normalizing inter-residue distance d0 for reference
    ...
    nTDP2OutputAlnDataPart3,
    //chain-specific statistics:
    dp2oadRMSD_C = nTDP2OutputAlnDataPart3,//RMSD
    dp2oadScoreQ_C,//TM-score normalized by the query length
    dp2oadScoreR_C,//TM-score normalized by the reference length
    dp2oadD0Q_C,//Normalizing inter-residue distance d0 for query
    dp2oadD0R_C,//Normalizing inter-residue distance d0 for reference
    dp2oad2ScoreQ_C,//2TM-score normalized by the query length
    dp2oad2ScoreR_C,//2TM-score normalized by the reference length
    nTDP2OutputAlnData
};
...
//NOTE: nTFilterData <= nDevGlobVariables!
enum TFilterData {
    ...
    fdNewComplexIndex,//new complex index (inclusive /prefix sum/)
    fdNewComplexAddress,//new complex address (inclusive /prefix sum/)
    nTFilterData
};
