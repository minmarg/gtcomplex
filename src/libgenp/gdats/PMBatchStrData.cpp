...
void PMBatchStrData::Sort()
{
    ...
    //NOTE: field pps2DCpxI is not updated; complexes are not sorted!
...
// -------------------------------------------------------------------------
// -------------------------------------------------------------------------
// ComplexComplete: complete filling the fields associated with complex 
// processing (after finalization)
//
void PMBatchStrData::ComplexComplete()
{
    MYMSG("PMBatchStrData::ComplexComplete",6);
    static const std::string preamb = "PMBatchStrData::ComplexComplete: ";
    static const int bchains = CLOptions::GetB_CHAINS();
    // size_t npositswrt = GetNoPositsWritten();//#positions written
    size_t nstructswrt = GetNoStructsWritten();//#structures written

    if(tmpclustfilendxs_.size() != tmpcomplexndxs_.size() ||
       tmpclustfilendxs_.size() != tmpcluststrndxs_.size() ||
       tmpclustfilendxs_.size() < nstructswrt)
        throw MYRUNTIME_ERROR2(
        preamb + "Inconsistent file index size.", CRITICAL);

    if(nstructswrt < 1) return;

    int nc = 0;
    INTYPE strlen = GetFieldAt<INTYPE,pps2DLen>(0);
    LNTYPE straddr = GetFieldAt<LNTYPE,pps2DDist>(0);
    INTYPE strtype = GetFieldAt<INTYPE,pmv2D_Ins_Ch_Ord>(straddr);
    std::string filendxprv = tmpcomplexndxs_[0];
    size_t nprv = 0;
    // LNTYPE clxaddrprv = 0;
    INTYPE clxtype = GetWatermarkedOnType(GetMoleculeType(strtype), strlen);

    SetFieldAt<INTYPE,pps2DCpxI>(0, nc);

    //fill in the complex fields:
    for(size_t n = 1; n < nstructswrt; n++) {
        strlen = GetFieldAt<INTYPE,pps2DLen>((int)n);
        straddr = GetFieldAt<LNTYPE,pps2DDist>((int)n);
        strtype = GetFieldAt<INTYPE,pmv2D_Ins_Ch_Ord>(straddr);//1st position
        const std::string& filendx = tmpcomplexndxs_[n];

        if(filendx != filendxprv || bchains) {
            // SetFieldAt<INTYPE,pcx2DN>(nc, straddr - clxaddrprv);
            // SetFieldAt<LNTYPE,pcx2DDstN>(nc, clxaddrprv);
            SetFieldAt<INTYPE,pcx2DN>(nc, (INTYPE)(n - nprv));
            SetFieldAt<LNTYPE,pcx2DDstN>(nc, (LNTYPE)nprv);
            SetFieldAt<INTYPE,pcx2DType>(nc, clxtype);//(clxtype <= 0)? -1: 0);
            nc++;
            nprv = n;
            // clxaddrprv = straddr;
            clxtype = 0;
        ...
        SetFieldAt<INTYPE,pps2DCpxI>((int)n, nc);
        CmbWatermarkOnType(GetMoleculeType(strtype), strlen, &clxtype);
        filendxprv = filendx;
    }

    //take account of the last structure:
    // SetFieldAt<INTYPE,pcx2DN>(nc, straddr + strlen - clxaddrprv);
    // SetFieldAt<LNTYPE,pcx2DDstN>(nc, clxaddrprv);
    SetFieldAt<INTYPE,pcx2DN>(nc, (INTYPE)(nstructswrt - nprv));
    SetFieldAt<LNTYPE,pcx2DDstN>(nc, (LNTYPE)nprv);
    SetFieldAt<INTYPE,pcx2DType>(nc, clxtype);//(clxtype <= 0)? -1: 0);
    nc++;
    //set the end addresses:
    bdbCpmend_[pcx2DN] = (char*)(((INTYPE*)(bdbCpmbeg_[pcx2DN])) + nc);
    bdbCpmend_[pcx2DDstN] = (char*)(((LNTYPE*)(bdbCpmbeg_[pcx2DDstN])) + nc);
    bdbCpmend_[pcx2DType] = (char*)(((INTYPE*)(bdbCpmbeg_[pcx2DType])) + nc);
}



// -------------------------------------------------------------------------
// PostProcess: post-process all chunk data having it compiled and finalized
//
void PMBatchStrData::PostProcess()
{
    ...
    //     return;
    // }
    //sort (of chains) is not suitable for complexes
    ComplexComplete();
}
    ...
    const unsigned int* filterdata)
{
    ...
    const unsigned int ndbcpxs = PMBatchStrData::GetNoComplexes(bdbpmbeg, bdbpmend);
    ...
    if(complexes)
        for(unsigned int n = 0; n < ndbcpxs; n++) {
            // unsigned int nchns = PMBatchStrData::GetFieldAt<INTYPE,pcx2DN>(bdbpmbeg, n);
            // unsigned int address = PMBatchStrData::GetAddressAt(bdbpmbeg, n);
            //NOTE: indices and addresses calculated by inclusive prefix sum!
            unsigned int newndx = filterdata[ndbstrs * fdNewComplexIndex + n];
            unsigned int newaddr = filterdata[ndbstrs * fdNewComplexAddress + n];
            if(newndx == 0) continue;
            //adjust the index appropriately (NOTE: newaddr is already valid):
            newndx--;
            //copy complex-specific fields to a different location in the same address space;
            ss_assigner.assign<INTYPE,pcx2DN>(newndx, n);
            ss_assigner.assign<INTYPE,pcx2DType>(newndx, n);
            ((LNTYPE*)(bdbpmbeg[pcx2DDstN]))[newndx] = newaddr;
            bdbpmend[pcx2DDstN] = (char*)(((LNTYPE*)(bdbpmbeg[pcx2DDstN])) + newndx + 1);
        }
    ...
    //adjust backreferences to complex ids: update the structure-specific pps2DCpxI field:
    if(complexes) {
        const unsigned int ndbcpxs2 = PMBatchStrData::GetNoComplexes(bdbpmbeg, bdbpmend);
        const unsigned int ndbstrs2 = PMBatchStrData::GetNoStructs(bdbpmbeg, bdbpmend);
        for(unsigned int n = 0; n < ndbcpxs2; n++) {
            unsigned int cpxnchns = 
                (unsigned int)PMBatchStrData::GetFieldAt<INTYPE,pcx2DN>(bdbpmbeg, n);
            unsigned int cpxdstn = PMBatchStrData::GetFieldAt<LNTYPE,pcx2DDstN>(bdbpmbeg, n);
            if(ndbstrs2 < cpxdstn + cpxnchns)
                throw MYRUNTIME_ERROR2(preamb + "Invalid #reference structures.", CRITICAL);
            unsigned int k = cpxdstn;
            for(; k < cpxdstn + cpxnchns; k++) ((INTYPE*)(bdbpmbeg[pps2DCpxI]))[k] = n;
            bdbpmend[pps2DCpxI] = (char*)(((INTYPE*)(bdbpmbeg[pps2DCpxI])) + k);
        }
    }
...
void PMBatchStrData::CopyFrom(const PMBatchStrData& bsd)
{
    ...
    const size_t nstructsovhd = bsd.GetNoStructsOvhd();
    if(maxnstrs_ + 1 < ndbstrs + nstructsovhd || nbdbCptrdescs_ + 1 < ndbstrs + nstructsovhd)
        throw MYRUNTIME_ERROR2(preamb + "Too many structures.", CRITICAL);
    if(maxdatalen_ < nposits)
        throw MYRUNTIME_ERROR2(preamb + "Too many total positions.", CRITICAL);
    ...
    for(; n < pps2DStrFlds; n++) {
        ...
        memcpy(bdbCpmend_[n], bsd.bdbCpmend_[n], TPM2DVectorFieldSize::szvfs_[n] * nstructsovhd);
    ...
    //ovhd:
    for(size_t di = ndbstrs; di < ndbstrs + nstructsovhd; di++) {
        ptrdescs[di] = bdbCdescs_.get() + di * PMBSdatDEFDESCLEN;
        strncpy(ptrdescs[di], bsdptrdescs[di], PMBSdatDEFDESCLEN);
    }
    //}}
    //file indices for clustering:
    tmpcomplexndxs_ = bsd.tmpcomplexndxs_;
...
void PMBatchStrData::AllocateSpaceForData()
{
    ...
    if(szbdbCdata_ < requestedsize)
    {
        ...
        tmpcomplexndxs_.reserve(maxnstrs_+1);
    ...
    const size_t nagents)
{
    ...
    //fill in temp. vectors for consistency
    tmpcomplexndxs_.clear();
    tmpclustfilendxs_.clear();
    tmpcluststrndxs_.clear();
    INTYPE cpxndxprv = 0;
    size_t filendx = 0;
    std::string cpxmodid = std::to_string(filendx);
    size_t nstructswrt = GetNoStructsWritten();//#structures written
    for(size_t n = 0; n < nstructswrt; n++) {
        const INTYPE cpxndx = GetFieldAt<INTYPE,pps2DCpxI>((int)n);
        if(cpxndx != cpxndxprv) {
            filendx++;
            cpxmodid = std::to_string(filendx);
        }
        cpxndxprv = cpxndx;
        tmpcomplexndxs_.push_back(cpxmodid);
        tmpclustfilendxs_.push_back(filendx);
        tmpcluststrndxs_.push_back(0);
    }
...
void PMBatchStrData::Print() const
{
    ...
    for(size_t n = 0; n < nstts; n++) {
        ...
        // int scpi = ((INTYPE*)(bdbCpmbeg_[pps2DCpxI]))[n];
...
void PMBatchStrData::Print(char* const * const bdbpmbeg, char* const * const bdbpmend)
{
    ...
    for(size_t n = 0; n < nstts; n++) {
        ...
        // int scpi = ((INTYPE*)(bdbpmbeg[pps2DCpxI]))[n];
