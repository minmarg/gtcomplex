...
:
    ...
    masterfilecounter_(masterfilecounter),
...
void TdDataReader::Execute( void* )
{
    ...
    try {
        ...
        while(1) {
            ...
            const size_t ntotqcpxs = ntotqcpxs_;
...
bool TdDataReader::GetData(
    size_t chunkdatasize, size_t chunkdatalen, size_t chunknstrs,
    const size_t ntotqcpxs, const int queryblocks,
    char* const * const * const querypmbegs,
    char* const * const * const querypmends)
{
    ...
    char* const * const * const querypmends)
{
    ...
    pbdbC->AllocateSpace(chunkdatasize, chunkdatalen, chunknstrs, chunkncpxs_);
    ...
    char* const * const * const querypmends)
{
    ...
    if(endofdata_) {//
        ...
        if(!(eodclustcached_ && bdbC4CC.FieldTypeValid() && !bdbC4CC.GetNoPositsOvhd())) {
            ndxpacketfile_ = ndxstartwith_;//reset
            //NOTE: clear data buffer upon completion to avoid processing the same data twice!
            bdbCstruct_[nextndx].AllocateSpace(chunkdatasize, chunkdatalen, chunknstrs, chunkncpxs_);//**
            return;//**
        }
    }
    else {
    ...
    char* const * const * const querypmends)
{
    ...
    if(ndxpacketfile_ < ndxlastpacketfile_) {
        ...
        if(readdata) {
            ...
            bdbC4CC.AllocateSpace(chunkdatasize, chunkdatalen, chunknstrs, chunkncpxs_);
        ...
        for(int i = 0; i < nstructswrt; i++) {
            ...
            if(type == INT_MAX) {
                ...
                if(glbndx < (int)ntotqcpxs) bdbC4CC.SetFieldAt<INTYPE,pps2DType>(i, glbndx);
            }
        }
