...
void PMBatchStrDataIndex::MakeIndex(const PMBatchStrData& strs)
{
    ...
    size_t nstts = complexes? strs.GetNoComplexes(): strs.GetNoStructsWritten();
    ...
    for(size_t n = 0; n < nstts; n++)
    {
        ...
        if(complexes) {
            int nchains = (int)strs.GetFieldAt<INTYPE,pcx2DN>(n);//#complex chains 
            int nchainsDst = (int)strs.GetFieldAt<LNTYPE,pcx2DDstN>(n);//#chains up to complex n
            len = 
                (int)PMBatchStrData::GetLengthAt(strs.bdbCpmbeg_, nchainsDst + nchains - 1) +
                (int)PMBatchStrData::GetAddressAt(strs.bdbCpmbeg_, nchainsDst + nchains - 1) -
                (int)PMBatchStrData::GetAddressAt(strs.bdbCpmbeg_, nchainsDst);
        }
