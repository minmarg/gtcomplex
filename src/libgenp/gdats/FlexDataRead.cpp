    ...
    char* const * const * const querypmends)
{
    ...
    while(!EndOfData() && readon) {
        if(Eof()) {
            ...
            if(clustmaster_) masterfilecounter_ = currentfilendx_;
            SetDbMetaData();
        }
    ...
    while(!Eof()) {
        ...
        if(prevchid != chid) {
            ...
            if(bsd.ContainsDataLast() && (cond1 || cond2))
            {
                ...
                bool increment =
                    (!cond1 && complexes && prevchid && splitopt >= CLOptions::issaByChain)? false: true;
                        ...
                        usechid, usemodn,  queryblocks, querypmbegs, querypmends))
                {
                    ...
                    case PMBatchStrData::pmbsdfLimits: if(increment) lfIncStr(); return false;
                    case PMBatchStrData::pmbsdfWritten: if(increment) lfIncStr(); break;
                }
