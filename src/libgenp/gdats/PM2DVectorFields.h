...
//fields and their indices of the 2D vector:
//complex-level fields:
        pcx2DN,//number of complex chains; [SHORT INT]
        pcx2DDstN,//total number of chains/structures up to this complex; [UINT]
        pcx2DType,//complex type (protein, NA; the sign bit); [SHORT INT]
// //--------------------------------------------------------------------------
// //chain-level fields to enable complex processing iteratively:
// //NOTE: these consistently index the chains across different complexes/structures.
//         pxc2DLen,//structure length; [SHORT INT]
//         pxc2DDist,//total number of positions up to this structure; [UINT]
//         pxc2DType_ChNo,//structure type (protein, NA; the sign bit) and chain number (the rest); [SHORT INT]
//--------------------------------------------------------------------------
//structure/chain-specific data:
//NOTE: pps2DLen and pps2DDist have to be adjacent for efficient reading in the device!
        pps2DLen,//structure length; [SHORT INT]
        pps2DDist,//total number of positions up to this structure; [UINT]
        pps2DCpxI,//serial number of complex this structure belongs to; [SHORT INT]
        pps2DType,//reserved for clustering; used to be structure type (protein, NA); [SHORT INT]
...
// max number of structures is unlikely to be processed in parallel simultaneously (on both CPU and GPU)
        pmv2DCoords,//coordinates; pmv2DNoElems fields in total
        ...
        pmv2DResNumber,//residue serial number (reserved for potential structure output) [SHORT INT]
        pmv2Drsd,//residue letter; [CHAR]
        pmv2Dss,//secondary structure (SS); [CHAR]
//}}
        pmv2DTotFlds//total number of fields
};
enum {
        pcx2DCpxFlds = pcx2DType + 1,//total number of complex-specific fields
