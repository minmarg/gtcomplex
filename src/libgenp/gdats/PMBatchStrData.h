...
enum PMBatchStrDataCnsts {
    PMBSSort = 0,//flag (0|1) of sorting chunk structures by length
    PMBSdatalignment = CACHECLINESIZE,
    PMBSdatDEFDESCLEN = DEFAULT_DESCRIPTION_LENGTH,//default description length (>4)
    PMBSmaxnocomplexchains = 2048,//max allowed #chains within a complex
...
//         memset( szpm2dvfovhd_, 0, pmv2DTotFlds * sizeof(size_t));
    ...
    // *** METHODS ***
    void AllocateSpace(size_t chunkdatasize, size_t chunkdatalen, size_t chunknstrs, int chunkncpxs = -1);
    ...
    //post process data once all chunk data has been compiled
    void PostProcess();
...
private:
    ...
    //complete filling the fields associated with complexes
    void ComplexComplete();
...
public:
    ...
    //get the min/max lengths of the structures written (NOTE:pointers assumed valid):
    static void GetMinMaxLengths(
        const char* const * const bdbpmbeg, const char* const * const bdbpmend,
        size_t& minlen, size_t& maxlen)
    {
        // static const int complexes = (CLOptions::GetB_CHAINS() == 0);
        // if(PMBSSort && !complexes) {
        //     minlen = (size_t)(((INTYPE*)(bdbpmend[pps2DLen]))[-1]);
        //     maxlen = (size_t)(*(INTYPE*)(bdbpmbeg[pps2DLen]));
        //     return;
        // }
        minlen = (1<<20); maxlen = 0;
        for(INTYPE* p = (INTYPE*)(bdbpmbeg[pps2DLen]); p < (INTYPE*)(bdbpmend[pps2DLen]); p++) {
            if((size_t)(*p) < minlen) minlen = (size_t)(*p);
            if((size_t)(*p) > maxlen) maxlen = (size_t)(*p);
        }
    }

    //get the min/max lengths of the complexes written (NOTE:pointers assumed valid):
    static void GetMinMaxComplexLengths(
        const char* const * const bdbpmbeg, const char* const * const bdbpmend,
        size_t& minlen, size_t& maxlen)
    {
        minlen = (1<<20); maxlen = 0;
        const int ncmpxs = GetNoComplexes(bdbpmbeg, bdbpmend);
        for(int n = 0; n < ncmpxs; n++) {
            const size_t cmpxlen = GetComplexLengthAt(bdbpmbeg, n);
            if(cmpxlen < minlen) minlen = cmpxlen;
            if(cmpxlen > maxlen) maxlen = cmpxlen;
        }
    }

    //complex length at position/index n (NOTE:pointers and indices assumed valid):
    static size_t GetComplexLengthAt(const char* const * const bdbpmbeg, int n) {
        int nchains = (int)GetFieldAt<INTYPE,pcx2DN>(bdbpmbeg, n);//#complex chains 
        int nchainsDst = (int)GetFieldAt<LNTYPE,pcx2DDstN>(bdbpmbeg, n);//#chains up to complex n
        const size_t cmpxlen = 
            GetLengthAt(bdbpmbeg, nchainsDst + nchains - 1) +
            GetAddressAt(bdbpmbeg, nchainsDst + nchains - 1) -
            GetAddressAt(bdbpmbeg, nchainsDst);
        return cmpxlen;
    }

    //get min #chains across all complexes (NOTE:pointers assumed valid):
    static size_t GetMinNComplexChains(
        const char* const * const bdbpmbeg, const char* const * const bdbpmend)
    {
        INTYPE minnch = (1<<20);
        for(INTYPE* p = (INTYPE*)(bdbpmbeg[pcx2DN]); p < (INTYPE*)(bdbpmend[pcx2DN]); p++)
            if((*p) < minnch) minnch = *p;
        return (size_t)(minnch);
    }

    //get max #chains across all complexes (NOTE:pointers assumed valid):
    static size_t GetMaxNComplexChains(
        const char* const * const bdbpmbeg, const char* const * const bdbpmend)
    {
        // static const int complexes = (CLOptions::GetB_CHAINS() == 0);
        // if(!complexes) return 1;
        INTYPE maxnch = 0;
        for(INTYPE* p = (INTYPE*)(bdbpmbeg[pcx2DN]); p < (INTYPE*)(bdbpmend[pcx2DN]); p++)
            if((*p) > maxnch) maxnch = *p;
        return (size_t)(maxnch);
    }
    ...
    //#complexes written in the buffers (NOTE:pointers assumed valid):
    static size_t GetNoComplexes(const char* const * const bdbpmbeg, const char* const * const bdbpmend) {
        return (size_t)(bdbpmend[pcx2DN]-bdbpmbeg[pcx2DN]) / SZINTYPE;
    }

    size_t GetNoComplexes() const {//#complexes written
        return (size_t)(bdbCpmend_[pcx2DN]-bdbCpmbeg_[pcx2DN]) / SZINTYPE;
    }
    ...
    size_t GetNoStructsOvhd() const {//#structures in the overhead
        return (size_t)(bdbCpmendovhd_[pps2DLen]-bdbCpmend_[pps2DLen]) / SZINTYPE;
    }
...
private:
    void SetMaxDataLimits(
        size_t maxdatasize, size_t maxdatalen, size_t maxnstrs, int maxncpxs)
    {
    ...
    size_t GetPMDataSizeOvhd() const;

    void AdjustEndPtrs(size_t ndx);

    void ResetPtrs() {//reset end pointers
        for(int f = 0; f < pmv2DTotFlds; f++)
            bdbCpmendovhd_[f] = bdbCpmend_[f] = bdbCpmbeg_[f];
    }
...
public:
    std::vector<std::string> tmpcomplexndxs_;//temporary complex index vector
...
//     size_t szcrntstrs_;//#structures currently in bdbCdata_
    ...
    int maxncpxs_;//max #complexes allowed for bdbCdata_ to contain
...
void PMBatchStrData::AllocateSpace(
    size_t chunkdatasize, size_t chunkdatalen, size_t chunknstrs, int chunkncpxs)
{
    //the following comes first:
    SetMaxDataLimits(chunkdatasize, chunkdatalen, chunknstrs, chunkncpxs);
...
// -------------------------------------------------------------------------
// GetPMDataSizeOvhd: get the overhead size
inline
size_t PMBatchStrData::GetPMDataSizeOvhd() const
{
    MYMSG("PMBatchStrData::GetPMDataSizeOvhd",6);
    size_t size = 0;
    for(int n = 0; n < pmv2DTotFlds; n++)
        size += (size_t)(bdbCpmendovhd_[n]-bdbCpmend_[n]);
    return size;
}
...
// -------------------------------------------------------------------------
// AdjustEndPtrs: adjust only end pointers to be located at structure ndx
inline
void PMBatchStrData::AdjustEndPtrs(size_t ndx)
{
    if(GetNoStructsWritten() < ndx)
        throw MYRUNTIME_ERROR2("PMBatchStrData::AdjustEndPtrs: Invalid index.", CRITICAL);
    int f = 0;
    for(; f < pps2DStrFlds; f++)
        bdbCpmend_[f] = bdbCpmbeg_[f] + TPM2DVectorFieldSize::szvfs_[f] * ndx;
    size_t dist = ndx
        ? (size_t)GetFieldAt<LNTYPE,pps2DDist>(ndx-1)
        + (size_t)GetFieldAt<INTYPE,pps2DLen>(ndx-1)
        : 0;
    for(; f < pmv2DTotFlds; f++)
        bdbCpmend_[f] = bdbCpmbeg_[f] + TPM2DVectorFieldSize::szvfs_[f] * dist;
}
    ...
    FPTYPE coords[pmv2DNoElems])
{
    ...
    {
        //NOTE: leave complex data arrangement upon limits to the finalizing function!
        //NOTE: this also truncates longer chains!
        if(complexes) return true;
    ...
    char* const * const * const /* querypmends */)
{
    ...
    const bool chaintoolarge = (maxdatasize_ < szstruct + szalign) || (maxdatalen_ < nposits);//allocation too small
    //NOTE: file index without a model number can be equal to another index with a model!
    const std::string cpxmodid = std::to_string(filendx) + '\x6' + strmodel + '\x6';

    if(nposits < 1) {
        FallbackOvhdPtrs();
        if(complexes && tmpcomplexndxs_.size() && tmpcomplexndxs_.back() == cpxmodid) {
            //NOTE: return the written flag if the complex is non-empty for global indices!
            warning((std::string("No atoms selected: '") + strchain + "': " + description).c_str());
    ...
    if(nposits < devminrlength) {
        ...
        if(complexes && tmpcomplexndxs_.size() && tmpcomplexndxs_.back() == cpxmodid) {
            //NOTE: return the written flag if the complex is non-empty for global indices!
            warning((std::string("Chain too short: '") + strchain + "': " + description).c_str());
            return pmbsdfWritten;
        }
        return pmbsdfShort;
    }

    if(!complexes && chaintoolarge) {
        //fallback, abandon the structure
        FallbackOvhdPtrs();
    ...
    *(LNTYPE*)(bdbCpmendovhd_[pps2DDist]) = (LNTYPE)npositswrt;
    //NOTE: the [pps2DCpxI] field updated on completion of the complexes: ComplexComplete!
    ...
    for(int f = 0; f < pps2DStrFlds; f++)
        bdbCpmendovhd_[f] += TPM2DVectorFieldSize::szvfs_[f];

    tmpcomplexndxs_.push_back(cpxmodid);
    tmpclustfilendxs_.push_back(filendx);
    tmpcluststrndxs_.push_back(strndx);

    int nchains = 0, ncpxs = 1, nn = (int)tmpcomplexndxs_.size() - 1;
    for(; 0 <= nn && tmpcomplexndxs_[nn] == cpxmodid; nn--, nchains++);
    for(; 0 <= nn; nn--) if(tmpcomplexndxs_[nn] != tmpcomplexndxs_[nn+1]) ncpxs++;

    if(maxdatasize_ < szstructswrt + szstruct + szalign || 
       maxdatalen_ < npositswrt + nposits || 
       maxnstrs_ < nstructswrt + 1 ||
      (complexes && chaintoolarge) ||
      (complexes && 0 < maxncpxs_ && maxncpxs_ < ncpxs) ||
      (complexes && PMBSmaxnocomplexchains < nchains))
    {
        ...
        *(LNTYPE*)(bdbCpmend_[pps2DDist]) = 0;
        if(!complexes) return pmbsdfLimits;
        //if all chains represent one complex, it is too large:
        if(tmpcomplexndxs_.size() != nstructswrt + 1)
            throw MYRUNTIME_ERROR2(preamb + "Inconsistent file index size.", CRITICAL);
        if(tmpcomplexndxs_[0] == cpxmodid) {
            ResetPtrs();
            tmpcomplexndxs_.clear();
            tmpclustfilendxs_.clear();
            tmpcluststrndxs_.clear();
            return pmbsdfAbandoned;
        }
        //for complexes, backtrace and adjust end pointers and distances for all complex chains
        size_t n = tmpcomplexndxs_.size() - 1;
        LNTYPE dist = 0;
        //NOTE: n>0 having the next loop completed---ensured by the above condition!
        for(; /* n && */tmpcomplexndxs_[n] == cpxmodid; n--);
        AdjustEndPtrs(n+1);
        for(LNTYPE incd = 0, incl = 0;
            bdbCpmend_[pps2DDist] + incd < bdbCpmendovhd_[pps2DDist];
            incd += TPM2DVectorFieldSize::szvfs_[pps2DDist],
            incl += TPM2DVectorFieldSize::szvfs_[pps2DLen])
        {
            *(LNTYPE*)(bdbCpmend_[pps2DDist] + incd) = dist;
            dist += (LNTYPE)(*(INTYPE*)(bdbCpmend_[pps2DLen] + incl));
        }
        //complex too large to be kept:
        if(maxdatalen_ < (size_t)dist || PMBSmaxnocomplexchains < nchains || chaintoolarge) {
            FallbackOvhdPtrs();
            tmpcomplexndxs_.erase(tmpcomplexndxs_.begin() + n+1, tmpcomplexndxs_.end());
            tmpclustfilendxs_.erase(tmpclustfilendxs_.begin() + n+1, tmpclustfilendxs_.end());
            tmpcluststrndxs_.erase(tmpcluststrndxs_.begin() + n+1, tmpcluststrndxs_.end());
            return pmbsdfAbandoned;
        }
        return pmbsdfLimits;
    }
...
bool PMBatchStrData::FieldTypeValid() const
{
    ...
    //NOTE: INT_MAX and -1 signify unprocessed structures and skipped by query, respectively!
    for(int i = 0; i < nstructswrt; i++) {
        const int globndx = GetFieldAt<INTYPE,pps2DType>(i);
        if(globndx < 0) continue;
        if(globndx == INT_MAX) return false;
    }
    return true;
}
...
PMBatchStrData::CopyOvhdTo(PMBatchStrData& bsd) const
{
    ...
    size_t szstruct = GetPMDataSizeOvhd();//PMBatchStrData::GetPMDataSize1(nposits);//size of data to be written
    size_t szstructswrtbsd = bsd.GetPMDataSize();//PMBatchStrData::GetPMDataSize1(npositswrtbsd);//size of data written
    size_t nstructswrtbsd = bsd.GetNoStructsWritten();//number of structures written
    size_t nstructs = GetNoStructsOvhd();//#structures to be written
    static constexpr size_t szalign = PMBSdatalignment * pmv2DTotFlds;//max size for data alignment
    ...
    if(bsd.maxdatasize_ < szstructswrtbsd + szstruct + szalign || 
       ...
       bsd.maxnstrs_ < nstructswrtbsd + nstructs)
    {
    ...
    size_t n = 0;
    for(; n < pps2DStrFlds; n++) {
        memcpy(bsd.bdbCpmend_[n], bdbCpmend_[n], TPM2DVectorFieldSize::szvfs_[n] * nstructs);
        bsd.bdbCpmend_[n] += TPM2DVectorFieldSize::szvfs_[n] * nstructs;
    ...
    size_t nstructswrt = GetNoStructsWritten();//number of structures written
    for(n = 0; n < nstructs; n++)
        strcpy(bsdptrdescs[nstructswrtbsd + n], ptrdescs[nstructswrt + n]);
    //}}

    if(tmpclustfilendxs_.size() != tmpcomplexndxs_.size() ||
       tmpclustfilendxs_.size() != tmpcluststrndxs_.size() ||
       tmpclustfilendxs_.size() != nstructswrt + nstructs)
        throw MYRUNTIME_ERROR2( 
        preamb + "Inconsistent file index size.", CRITICAL);

    for(auto it = tmpcomplexndxs_.begin() + nstructswrt; it < tmpcomplexndxs_.end(); it++)
        bsd.tmpcomplexndxs_.push_back(*it);

    for(auto it = tmpclustfilendxs_.begin() + nstructswrt; it < tmpclustfilendxs_.end(); it++)
        bsd.tmpclustfilendxs_.push_back(*it);

    for(auto it = tmpcluststrndxs_.begin() + nstructswrt; it < tmpcluststrndxs_.end(); it++)
        bsd.tmpcluststrndxs_.push_back(*it);
