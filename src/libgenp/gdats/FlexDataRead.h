...
public:
    ...
    bool EndOfData() {
        ...
        return (masterfilecounter_ < currentfilendx_);
    ...
    int GetGlobalId(size_t filendx, int strndx) const {
        ...
        if((int)globalids_[filendx].size() <= strndx)
            //NOTE: -1 indicates structures skipped by query!
            return (masterfilecounter_ <= filendx)? INT_MAX: -1;
...
private:
    ...
    size_t& masterfilecounter_;//master's file counter for clustering
