...
public:
        ...
        size_t /* sz_alndata */)
    {
        ...
        const int nqcpxs = PMBatchStrData::GetNoComplexes(querypmbeg, querypmend);

        if(nqystrs <= 0 || //mstr_set_nmaxchunkqueries_ < nqystrs ||
           nqcpxs <= 0 || mstr_set_nmaxchunkqueries_ < nqcpxs) {
            lfError();
        ...
        //NOTE: for --chains, complexes represent individual chains!
        const int nrfncpxs = (int)PMBatchStrData::GetNoComplexes(bdbCpmbeg, bdbCpmend);
        const int nrfnstrs = (int)PMBatchStrData::GetNoStructs(bdbCpmbeg, bdbCpmend);
        int ntotstrs = 0;

        for(int i = 0; i < nqcpxs; i++) {
            const int nqchns = (int)PMBatchStrData::GetFieldAt<INTYPE,pcx2DN>(querypmbeg, i);
            const int nqchnsDst = (int)PMBatchStrData::GetFieldAt<LNTYPE,pcx2DDstN>(querypmbeg, i);
            //qsn==qryglobndx:
            const int qsn = PMBatchStrData::GetFieldAt<INTYPE,pps2DType>(querypmbeg, nqchnsDst);
            //NOTE: qrysernrbeg represents chains whereas qsn complexes!
            if(qsn < qrycpxsernrbeg || qrycpxsernrbeg + nqcpxs <= qsn) {
                lfError();
                throw MYRUNTIME_ERROR(preamb + "Invalid query global identifier.");
            }
            const int vecndx = GetValidatedVecIndexForQrynum(qsn);
            const int lenqry = (int)PMBatchStrData::GetComplexLengthAt(querypmbeg, i);
            parts_qrs_[qsn]--;
            if(parts_qrs_[qsn] <= 0) {
                unsigned int len1 = (unsigned int)PMBatchStrData::GetLengthAt(querypmbeg, nqchnsDst);
                unsigned int addr = (unsigned int)PMBatchStrData::GetAddressAt(querypmbeg, nqchnsDst);
                const char* rsds = querypmbeg[pmv2Drsd] + addr;
                vec_querydesc_[vecndx] = querydesc[nqchnsDst];
                if(boutputseqs_) {
                    vec_queryseqn_[vecndx] = std::string(rsds, len1);
                    for(int c = 1; c < nqchns; c++) {
                        len1 = (unsigned int)PMBatchStrData::GetLengthAt(querypmbeg, nqchnsDst + c);
                        addr = (unsigned int)PMBatchStrData::GetAddressAt(querypmbeg, nqchnsDst + c);
                        rsds = querypmbeg[pmv2Drsd] + addr;
                        vec_queryseqn_[vecndx] += "*" NL;
                        vec_queryseqn_[vecndx] += std::string(rsds, len1);
                    }
                }
                ntotchains_ += nqchns;
            ...
            int soff = nDevGlobVariables * nqchnsDst;
            int nstrs = passedstats? passedstats[soff + dgvNPassedStrs]: 0;
            if(nstrs != nrfnstrs) {
                lfError();
                throw MYRUNTIME_ERROR(preamb + "Inconsistent number of reference chains in chunk.");
            }
            // #pragma omp simd
            for(int rn = 0; rn < nrfncpxs; rn++) {
                const int nrfnchns = (int)PMBatchStrData::GetFieldAt<INTYPE,pcx2DN>(bdbCpmbeg, rn);
                const int nrfncDst = (int)PMBatchStrData::GetFieldAt<LNTYPE,pcx2DDstN>(bdbCpmbeg, rn);
                const int globndx = PMBatchStrData::GetFieldAt<INTYPE,pps2DType>(bdbCpmbeg, nrfncDst);
                if(globndx < 0 || qrycpxsernrbeg + nqcpxs <= globndx) continue;
                const int lenrfn = PMBatchStrData::GetComplexLengthAt(bdbCpmbeg, rn);
                if(globndx == qsn && lenrfn != lenqry) {
                    lfError();
                    throw MYRUNTIME_ERROR(preamb + "Inconsistent structure ids.");
                }
                //distance:
                const float nalnres = (int)GetNAlignedResiduesComplex(
                                        h_results, nqchns, nrfnchns, nqchnsDst, nrfncDst, nrfnstrs);
                float tmscoreq = GetOutputAlnDataFieldComplex<float,dp2oadScoreQ>(h_results, i, nrfnstrs, rn);
                float tmscorer = GetOutputAlnDataFieldComplex<float,dp2oadScoreR>(h_results, i, nrfnstrs, rn);
                float rmsd = GetOutputAlnDataFieldComplex<float,dp2oadRMSD>(h_results, i, nrfnstrs, rn);
...
protected:
    ...
    template<typename T, int field>
    T GetOutputAlnDataFieldChain(const char* h_results, int gqrychnndx, int nrfnstrs, int dbstrndx) const
        {   //NOTE: gqrychnndx is the global query chain index within chunk
            return
                *(T*)((float*)h_results +
                    nTDP2OutputAlnData * (gqrychnndx * nrfnstrs + dbstrndx) + field);
        }

    template<typename T, int field>
    T GetOutputAlnDataFieldComplex(const char* h_results, int qrycpxndx, int nrfnstrs, int dbcpxndx) const
        {
            return
                *(T*)((float*)h_results +
                    nTDP2OutputAlnData * (qrycpxndx * nrfnstrs + dbcpxndx) + field);
        }

    int GetNAlignedResiduesComplex(
        const char* h_results, int qrycpxN, int rfncpxN, int qrycpxDstN, int rfncpxDstN, int nrfnstrs) const
    {
        int alnlencpx = 0, ngapscpx = 0;
        for(int q = 0; q < qrycpxN; q++)
            for(int r = 0; r < rfncpxN; r++) {
                const int qchn = qrycpxDstN + q;
                const int rchn = rfncpxDstN + r;
                //chainn assignment:
                float chnassg = GetOutputAlnDataFieldChain<float,dp2oadOrgStrNo>(h_results, qchn, nrfnstrs, rchn);
                if(chnassg == 0.0f) continue;
                int alnlen = (int)GetOutputAlnDataFieldChain<float,dp2oadAlnLength>(h_results, qchn, nrfnstrs, rchn);
                //NOTE: alignment length can be 0 due to pre-screening
                if(alnlen < 1) continue;
                int ngaps = (int)GetOutputAlnDataFieldChain<float,dp2oadNGaps>(h_results, qchn, nrfnstrs, rchn);
                alnlencpx += alnlen;
                ngapscpx += ngaps;
            }
        return (alnlencpx - ngapscpx);
    }
...
private:
    ...
    size_t ntotchains_;//total number of chains
    //}}
    //{{clustering data:
    //forward pointers:
