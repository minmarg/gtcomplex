...
//     }
    ...
    void PushPartOfResults( 
        int qrysernr,
        int qrysernrignoreto,
        int nqyposs,
        const std::string& qrydesc,
        const std::string& qryinfo,
        ...
        std::unique_ptr<std::vector<char*>> annotptrs)
    {
        ...
        if( --parts_qrs_[qrysernr] <= 0 ) {
            ...
            //trigger write to a file
            qrysernr_ = qrysernr;
            qrysernrignoreto_ = qrysernrignoreto;
            req_msg_ = wrtthreadmsgWrite;
            lck.unlock();
            cv_msg_.notify_all();
        }
        for(++qrysernr; qrysernr < qrysernrignoreto; qrysernr++)
            parts_qrs_[qrysernr]--;
    }
...
protected:
    ...
    void WriteQueryDescriptionJSON(FILE* fp,
        char* const buffer, const int szbuffer, char*& outptr, int& offset,
        char* tmpbuf, int /*sztmpbuf*/,
        const int qrylen,
...
private:
    ...
    std::vector<std::string> vec_qryinfo_;//query information
    ...
    int qrysernrignoreto_;//#chains comprising query qrysernr_
};
