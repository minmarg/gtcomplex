...
void TdAlnWriter::WriteResultsJSON()
{
    ...
    static const char* chainapattern = "chain_alignment_entry";
    ...
    const size_t nchns = CLOptions::GetO_NCHNS();
    std::string filename;
    ...
    WriteQueryDescriptionJSON(fp.get(),
        buffer_, szWriterBuffer, pb, size,
        ptmp, szWriterBuffer/*maxsize*/,
        vec_nqyposs_[qrysernr_],
        vec_qryinfo_[qrysernr_],
        vec_qrydesc_[qrysernr_], (int)dsclen);
    ...
    if(hitsfound)
    {
        ...
        for(size_t i = 0, nc = 0; i < p_finalindxs_->size() && i < nalns && (nchns? (nc < nchns): 1); i++) {
            const char* aln = 
            ...
            if(nchns) {
                for(const char* p = strstr(aln, chainapattern); p; nc++) p = strstr(p + 1, chainapattern);
                if(!last) last = (nchns <= nc);
            }
            BufferData(fp.get(),
                buffer_, szWriterBuffer, pb, size,
                aln, alnlen);
...
// maxwidth, max length of the query description allowed to be written;
//
void TdAlnWriter::WriteQueryDescriptionJSON( 
    FILE* fp,
    char* const buffer, const int szbuffer, char*& outptr, int& offset,
    ...
    const int maxwidth)
{
    ...
    FormatDescriptionJSON((p = tmpbuf), descopy.c_str(), descopy.size(), maxwidth, outpos);
    BufferData(fp,
        buffer, szbuffer, outptr, offset,
        tmpbuf, outpos);
    ...
    written = sprintf(tmpbuf,
            ...
            "    \"length\": %d,%s"
            "    \"n_chains\": %d,%s",
            NL,qrylen,NL,qrysernrignoreto_ - qrysernr_,NL);
    BufferData(fp,
        buffer, szbuffer, outptr, offset,
        tmpbuf, written);
    ...
    const bool found)
{
    ...
    written = sprintf(tmpbuf,
            "    ],%s"
            "    \"number_of_chains\": %zu,%s"
            "    \"number_of_residues\": %zu%s"
            "  },%s",
