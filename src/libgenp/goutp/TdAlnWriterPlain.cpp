    ...
    const int width )
{
    ...
    if(complexes)
        written = sprintf(outptr, "%s Query (%d residues, %d chain(s)):%s",
            NL, qrylen, qrysernrignoreto_ - qrysernr_, NL);
    else written = sprintf(outptr, "%s Query (%d residues):%s", NL, qrylen, NL);
    outptr += written;
    size += written;
    ...
    if(complexes) {
        if((posc = descopy.rfind(" Chn:")) != std::string::npos) {
    ...
    const bool clustering)
{
    ...
    written = sprintf(tmpbuf,"%13c%zu total chain(s)%s",' ',nentries,NL);
    BufferData(fp,
        buffer, szbuffer, outptr, offset,
        tmpbuf, written);
    ...
    written = sprintf(tmpbuf," Legend:%s"
        ...
        "RMSD, Root-mean-square deviation (A); NA, nucleic acid; Chn, Chain; (M), Model%s"
        "+, pairs of aligned residues within a distance of %.0f A%s"
        "Secondary structure: h, Helix; e, Strand; t, Turn%s"
        "(dot-bracket notation for nucleic acids)%s%s",
        NL,NL,NL,NL,NL,EQUIVALENCE_DISTANCE,NL,NL,NL,NL);
    BufferData(fp,
        buffer, szbuffer, outptr, offset,
        tmpbuf, written);

    written = sprintf(tmpbuf,"%61s|%5s|%11s|%11s|%s",
        "Query_length-normalized_TM-score","RMSD","#Identities","#Matched",NL);
    BufferData(fp,
        buffer, szbuffer, outptr, offset,
        tmpbuf, written);
    written = sprintf(tmpbuf,"%54s|%41s|%s",
        "Reference_length-normalized_TM-score","#Reference_chains",NL);
    BufferData(fp,
        buffer, szbuffer, outptr, offset,
        tmpbuf, written);
    written = sprintf(tmpbuf,"%6s|%40s|%25s|%11s|%17s|%s%s",
        "No.","Reference_description","#Aligned_residues","%Identities","Reference_length",NL,NL);
    BufferData(fp,
        buffer, szbuffer, outptr, offset,
        tmpbuf, written);
}
