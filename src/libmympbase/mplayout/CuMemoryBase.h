...
public:
    ...
    static size_t GetMaxQueriesPerChunk() {return (size_t)CLOptions::GetDEV_QRS_MAX_PER_CHUNK();}
    static size_t GetMaxAlnLength();
    static size_t GetMaxNFragSteps();
    static size_t GetMaxNFragStepsMem2();
...
// -------------------------------------------------------------------------
// GetMaxNFragStepsMem2: get max number of steps attributed to sections of
// working memory 2
inline
size_t CuMemoryBase::GetMaxNFragStepsMem2()
{
    static const int depth = CLOptions::GetC_DEPTH();
    static const int topndpscores = CLOptions::GetC_RECALCULATE();
    const size_t nconfigsections = 
        (depth <= CLOptions::csdDeep)? 3: ((depth <= CLOptions::csdHigh)? 2: 1);
    const size_t nconfigs = (topndpscores) * nconfigsections;
    return nconfigs;
}
...
size_t CuMemoryBase::GetSizeOfDPDiagScores(size_t maxdbposs) const
{
    ...
    const size_t cszalnment = GetMemAlignment();
    const size_t maxnqrsperchunk = GetMaxQueriesPerChunk();
    const size_t maxnquerychains = (size_t)(CLOptions::GetDEV_QRS_MAX_CHAINS());
    const size_t maxnsteps = mymax(GetMaxNFragSteps(), maxnquerychains);
    const size_t padding = 
        ((CUL2CLINESIZE < CUDP_2DCACHE_DIM_D_BASE)? CUDP_2DCACHE_DIM_D_BASE: CUL2CLINESIZE) * 2;
...
size_t CuMemoryBase::GetSizeOfDPBottomScores(size_t maxdbposs) const
{
    ...
    //NOTE: 200 at least should be used for spectral scores;
    //NOTE: using much smaller number; for ExecDPScore3264x, the bigger
    //NOTE: tmpdpalnpossbuffer buffer is used instead!
    const_cast<size_t&>(maxnsteps) = mymin((size_t)16, maxnsteps);
    const size_t padding = 
        ((CUL2CLINESIZE < CUDP_2DCACHE_DIM_D_BASE)? CUDP_2DCACHE_DIM_D_BASE: CUL2CLINESIZE) * 2;
    //NOTE: nTDPAGCDiagScoreSubsections when spectral score and affine gap costs are in use
    constexpr size_t nsubsections = nTDPDiagScoreSubsections;//nTDPAGCDiagScoreSubsections
...
size_t CuMemoryBase::GetSizeOfWrkMemory2(size_t ndbstrs) const
{
    ...
    const size_t maxnqrsperchunk = GetMaxQueriesPerChunk();
    const size_t maxnsteps = GetMaxNFragSteps();
    const size_t szwrkmem20 = 
            //calculated for each query-target structure pair:
            maxnqrsperchunk *
            (ndbstrs * maxnsteps * nTWorkingMemory2Vars) *
            sizeof(float);
    //NOTE: the wrkmem2 section is also used for multi-config chain assignments:
    //NOTE: size of wrkmem2 will accommodate the largest references with the max of 2048 chains:
    //NOTE: (szwrkmem20 = 1024 * 16) > (2/max query chains in stage_chor_identify_top_orientations_complex/ * 2048 * 3)
    //NOTE: > (szwrkmem21 = 100/min query chains for occupation/ * 32/min #configs/ * 3)!
    const size_t maxnquerychainstotal = (size_t)(CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK());
    const size_t nconfigs = GetMaxNFragStepsMem2();
    const size_t szwrkmem21 = 
            maxnquerychainstotal *
            (ndbstrs * nconfigs * nTWorkingMemory2VarsAssignment) *
            sizeof(float);
    size_t szwrkmem2 = mymax(szwrkmem20, szwrkmem21);
...
size_t CuMemoryBase::GetSizeOfDP2AlnData(size_t ndbstrs) const
{
    ...
    const size_t maxnquerychainstotal = (size_t)(CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK());
    return GetSizeOfDP2AlnData(maxnquerychainstotal, ndbstrs);
}
    ...
    size_t totqrsposs, size_t ndbposs, size_t ndbstrs, bool sssinuse) const
{
    ...
    const size_t maxnquerychainstotal = (size_t)(CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK());
    return GetSizeOfDP2Alns(maxnquerychainstotal, totqrsposs, ndbposs, ndbstrs, sssinuse);
}
    ...
    size_t totqrsposs, bool sssinuse ) const
{
    ...
    const size_t maxnquerychainstotal = (size_t)(CLOptions::GetDEV_QRS_TOTAL_CHAINS_PER_CHUNK());
    const size_t maxnsteps = mymax(GetMaxNFragSteps(), maxnquerychains);
    const size_t nconfigs2 = GetMaxNFragStepsMem2();
    const size_t padding = 
        ((CUL2CLINESIZE < CUDP_2DCACHE_DIM_D_BASE)? CUDP_2DCACHE_DIM_D_BASE: CUL2CLINESIZE) * 2;
    ...
    size_t deplen2 = (size_t)(
        ...
        ((float)/*1.0f*/maxnquerychainstotal +
            (float)(totqrsposs + maxnquerychainstotal + 1/*rounding*/) /
    ...
    size_t indeplen02 = 
       (size_t)INT_MAX * CLOptions::GetDEV_EXPCT_DBPROLEN() /
           (/*1*/maxnquerychainstotal * nconfigs2 * nTWorkingMemory2VarsAssignment);

    size_t indeplen0 = mymin(indeplen01, indeplen02);

    //independent maximum length derived from alignment statistics size requirements:
    //(evidently large)
    //size_t indeplen1 = 
    //    (size_t)INT_MAX * CLOptions::GetDEV_EXPCT_DBPROLEN() /
    //        (/*1*/maxnquerychainstotal * nTDP2OutputAlnData);
    ...
    //take minimum:
    size_t maxallwdposs = mymin(deplen1, deplen2);
    maxallwdposs = mymin(maxallwdposs, indeplen0);
    //maxallwdposs = mymin(maxallwdposs, indeplen1);
    //maxallwdposs = mymin(maxallwdposs, indeplen2);
    maxallwdposs = mymin(maxallwdposs, indeplen3);
    maxallwdposs = mymin(maxallwdposs, indeplen4);
    maxallwdposs = mymin(maxallwdposs, indeplen5);
    return maxallwdposs;
}
